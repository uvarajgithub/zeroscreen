...
function consolidatedShadowSummary(externalHealth: any = {}, underlying: UnderlyingId = "BANKNIFTY"): any {
  ...
  for (const strategy of availableStrategies) {
    ...
    for (const instrument of strategy.instruments || (["FUTURES", "OPTIONS"] as InstrumentType[])) {
      ...
      const currentContractTrades = currentContractDays.reduce((sum: number, d: any) => sum + (num(d.trades) ?? 0), 0) + (fields.inTrade && !todayHistory ? 1 : 0);
      ...
      const VARIANT_BT_JULY: Record<string, { FUTURES: { pnl: number; ret: number; trades: number }; OPTIONS: { pnl: number; ret: number; trades: number } }> = {
