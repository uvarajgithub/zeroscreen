...
function consolidatedShadowSummary(externalHealth: any = {}, underlying: UnderlyingId = "BANKNIFTY"): any {
  ...
  for (const strategy of availableStrategies) {
    ...
    for (const instrument of strategy.instruments || (["FUTURES", "OPTIONS"] as InstrumentType[])) {
      ...
      const monthDays = (monthlyHistory?.days || []).filter((d: any) => String(d.date || d.period || "").startsWith(currentMonth));
      const historyMonthPnl = monthDays.reduce((sum: number, d: any) => sum + (num(d.pnl) ?? 0), 0);
      ...
      const monthTrades = monthDays.reduce((sum: number, d: any) => sum + (num(d.trades) ?? 0), 0) + (fields.inTrade && !todayHistory ? 1 : 0);
