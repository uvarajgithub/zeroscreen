...
export async function initDb(): Promise<void> {
  ...
  await new Promise<void>((resolve, reject) => {
    db.serialize(() => {
      ...
      db.run("INSERT OR IGNORE INTO app_settings (key,value) VALUES ('paper_start_balance_user','100000')", () => {});
      db.run("INSERT OR IGNORE INTO app_settings (key,value) VALUES ('paper_start_balance_admin','1000000')", () => {});
...
export async function getAllUsers(): Promise<Omit<UserRow, "password">[]> {
  return dbAll<Omit<UserRow, "password">>(
    `SELECT u.id, u.name, u.email, u.role, u.created_at, COALESCE(u.is_blocked,0) AS is_blocked,
            COALESCE(pp.balance, 0) AS paper_balance
     FROM users u LEFT JOIN paper_portfolio pp ON pp.user_id = u.id
     ORDER BY u.created_at DESC`
  );
}

export async function blockUser(userId: number): Promise<void> {
  await dbRun("UPDATE users SET is_blocked=1 WHERE id=?", [userId]);
}
...
export async function setUserPaperBalance(userId: number, balance: number): Promise<void> {
  await dbRun("INSERT OR REPLACE INTO paper_portfolio (user_id,balance) VALUES (?,?)", [userId, balance]);
}
...
export async function getPaperPortfolio(userId: number, role?: string): Promise<{ balance: number }> {
  const startBal = role === "admin" ? 1000000 : 100000;
  const row = await dbGet<{ balance: number }>("SELECT balance FROM paper_portfolio WHERE user_id=?", [userId]);
  if (!row) {
    await dbRun("INSERT OR IGNORE INTO paper_portfolio (user_id,balance) VALUES (?,?)", [userId, startBal]);
...
export async function paperReset(userId: number, role?: string): Promise<void> {
  const startBal = role === "admin" ? 1000000 : 100000;
  await dbRun("DELETE FROM paper_positions WHERE user_id=?", [userId]);
  await dbRun("DELETE FROM paper_trades WHERE user_id=?", [userId]);
  await dbRun("INSERT OR REPLACE INTO paper_portfolio (user_id,balance) VALUES (?,?)", [userId, startBal]);
}
