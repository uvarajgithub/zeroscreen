 ...
 */
export type TrailFn = (sl: number, entry: number, dir: "CE" | "PE", peak: number) => number;

export const trailDefault: TrailFn = (sl, entry, dir, peak) => {
  let lock = 0;
...
export const trailLock50: TrailFn = (sl, entry, dir, peak) => {
...
export function processHybridCandle(
  ...
  if (state.inTrade) {
    ...
    // Trailing stop — driven by trailFn (TRAIL by default, LOCK50 for shadow)
    const hp = state.dir === "CE"
      ? current.high - state.entry
      : state.entry  - current.low;
    if (hp > state.peakProfit) {
      state.peakProfit = hp;
      const newSL = trailFn(state.sl, state.entry, state.dir!, state.peakProfit);
      state.sl = newSL;
    }
