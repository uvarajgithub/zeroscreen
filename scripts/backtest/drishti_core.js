// JS port of src/drishti_strategy.ts core functions, with configurable trail/SL params
// for scenario backtesting. Entry-signal and re-entry logic kept IDENTICAL to live source.
...
function findDrishtiEntry(todayCandles, prevDayCandles) {
  ...
  if (ctx === 'ABOVE_PDH') {
    ...
    const largeGapHoldingPdh = vsPDH > 150 && C0.close > PH;
    if (largeGapHoldingPdh) {
      const bullIdx = _firstBull(todayCandles, C0bp > 40 ? 0 : 1, 40);
      if (bullIdx >= 0 && todayCandles[bullIdx].close > PH)
        return at(bullIdx, 'CE', 'above_pdh_large_gap_follow_ce');
      return null;
    }
...
function findDrishtiReEntry(todayCandles, exitIdx, side, allowReverse, prevDayCandles) {
  const lastIdx = todayCandles.length - 1;
  if (lastIdx <= exitIdx) return null;

  const C0 = todayCandles[0];
  const PH = prevDayCandles && prevDayCandles.length > 0 ? _pdh(prevDayCandles) : 0;
  const PL = prevDayCandles && prevDayCandles.length > 0 ? _pdl(prevDayCandles) : 0;
  const latestClose = todayCandles[lastIdx].close;
  const blockPE = !!(C0 && PH > 0 && C0.open - PH > 150 && latestClose > PH);
  const blockCE = !!(C0 && PL > 0 && PL - C0.open > 150 && latestClose < PL);
  ...
  for (let i = exitIdx + 1; i <= lastIdx; i++) {
    ...
    if (!sameDirMatch) {
      if (side === 'CE' && !blockCE && b > 40) sameDirMatch = { idx: i, side, reason: 're_same_dir' };
      if (side === 'PE' && !blockPE && b < -40) sameDirMatch = { idx: i, side, reason: 're_same_dir' };
    }
    if (!revMatch && allowReverse) {
      if (revSide === 'CE' && !blockCE && b > 40) revMatch = { idx: i, side: revSide, reason: 're_reverse' };
      if (revSide === 'PE' && !blockPE && b < -40) revMatch = { idx: i, side: revSide, reason: 're_reverse' };
    }
    if (sameDirMatch && (!allowReverse || revMatch)) break;
  }
...
// Configurable trail update.
// trailGapFn(peakPts) -> trail gap in pts (replaces the fixed TRAIL_GAP=10 constant)
function updateDrishtiTrail(state, candle, isEOD, opts) {
  const { slPts, trailGapFn } = opts;
  ...
  if (favPts > peakPts) {
    peakPts = favPts;
    const gap = trailGapFn(peakPts);
    trailStop = peakPts >= gap ? peakPts - gap : -slPts;
  }
...
module.exports = {
  createDrishtiState, findDrishtiEntry, findDrishtiReEntry, updateDrishtiTrail,
};
