'use strict';
// bt_struct_reentry.js — structural break for BOTH entry AND re-entry
// Compare against bt_struct_break.js (struct entry + body% re-entry = 266,196 pts)
...
// ── Entry logic: identical to bt_struct_break.js ───────────────────────────
function findEntry(today, prev) {
  ...
  if (ctx === 'BELOW_PDL') {
    ...
    if (C0.high < PL) {
      if (today.length >= 2 && bp(today[1]) > 20) return at(1, 'CE', 'c1_bull');
      if (bp(today[1] || today[0]) < -20) return at(0, 'PE', 'no_rec');
  ...
  if (Math.abs(C0bp) > 55) {
    ...
    if (aligned) {
      if (today.length >= 2 && bp(today[1]) * C0bp < 0 && Math.abs(bp(today[1])) > 72) { const s = at(1, bp(today[1]) > 0 ? 'CE' : 'PE', 'trap'); if (s) return s; }
...
// ── Re-entry: STRUCTURAL BREAK version ─────────────────────────────────────
// Instead of body% >40%, we require close outside previous candle's range.
// Same-dir CE: curr.close > prev.high  |  Same-dir PE: curr.close < prev.low
// Reverse CE : curr.close > prev.high  |  Reverse PE : curr.close < prev.low
function findReEntry(today, exitIdx, side, allowReverse) {
  ...
  for (let i = exitIdx + 1; i <= lastIdx; i++) {
    const prevC = today[i - 1], curr = today[i];
    if (!sd) {
      if (side === 'CE' && curr.close > prevC.high) sd = { idx: i, side };
      if (side === 'PE' && curr.close < prevC.low)  sd = { idx: i, side };
    }
    if (!rd && allowReverse) {
      if (rev === 'CE' && curr.close > prevC.high) rd = { idx: i, side: rev };
      if (rev === 'PE' && curr.close < prevC.low)  rd = { idx: i, side: rev };
    }
    if (sd && (!allowReverse || rd)) break;
  }

  if (sd && rd) return sd.idx <= rd.idx ? sd : rd;
  return sd || rd || null;
}
...
async function main() {
  console.log('STRUCTURAL BREAK RE-ENTRY backtest (Jan 2021 – May 2026)...');
  ...
  console.log('\n════════════════════════════════════');
  console.log('STRUCT ENTRY + STRUCT RE-ENTRY Results');
  console.log('════════════════════════════════════');
  ...
  console.log('────────────────────────────────────');
  console.log('Compare: STRUCT entry + body% re-entry = 266,196 pts');
}
