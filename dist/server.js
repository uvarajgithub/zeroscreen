...
function render(d){
  ...
  if(PAGE==='trade-history'){
    ...
    function rangeLabel(range){return range==='latest-session'?'Latest Session':range==='weekly'?'Weekly':range==='monthly'?'Monthly':'Today'}
    ...
    function matchesSearch(t,q){q=String(q||'').trim().toLowerCase();if(!q)return true;return [t.date,t.time,t.symbol,t.side,t.qty,t.entry,t.exit,t.pnl,t.status,t.note,t.reason,t.source].join(' ').toLowerCase().includes(q)}
    ...
    root.innerHTML=header('Trade History','Analyze completed 10:30 futures trades by date, symbol, status, and outcome.')+scope+'<div id="thSummary" class="ws-summary"></div><section class="ws-card"><div class="ws-card-b"><div class="filter-bar trade-history-filters"><button class="seg active" data-th-range="today" type="button">Today</button><button class="seg" data-th-range="latest-session" type="button">Latest Session</button><button class="seg" data-th-range="weekly" type="button">Weekly</button><button class="seg" data-th-range="monthly" type="button">Monthly</button><select id="thType"><option value="all">All Trades</option><option value="winners">Winners</option><option value="losers">Losers</option><option value="buy">Buy</option><option value="sell">Sell</option><option value="closed">Closed</option><option value="rejected">Rejected</option></select><input id="thSearch" placeholder="Search..."><button id="thApply" class="btn primary" type="button">Apply Filter</button><button id="thReset" class="btn" type="button">Reset</button></div></div></section><div class="ws-grid"><div class="ws-main"><section class="ws-card"><div class="ws-card-h"><span>Trade History</span><span id="thCount" class="muted">0 records</span></div><div class="ws-card-b"><div class="ws-table-wrap"><table class="ws-table"><thead><tr><th>Date</th><th>Time</th><th>Symbol</th><th>Side</th><th class="right">Qty</th><th class="right">Entry</th><th class="right">Exit</th><th class="right">P&L</th><th>Status</th></tr></thead><tbody id="thBody"></tbody></table></div><div id="thTotals" class="workflow-line" style="margin-top:10px"></div><div class="ws-table-footer"><span id="thPageTotal">Showing 0-0 of 0</span><div class="ws-pager"><button id="thPrev" class="btn" type="button">Prev</button><span id="thPageInfo">Page 1 of 1</span><button id="thNext" class="btn" type="button">Next</button></div></div></div></section></div><aside class="ws-side"><section class="ws-card"><div class="ws-card-h"><span>Trade Insights</span></div><div id="thInsights" class="ws-card-b side-list"></div></section></aside></div></section>';
    const state={range:'today',type:'all',q:'',page:1};
    const pageSize=10;
    function drawTradeHistory(){const list=filteredTrades(state);const s=stats(list);const monthList=allTrades.filter(t=>inRange(t,'monthly'));const ms=stats(monthList);const pages=Math.max(1,Math.ceil(list.length/pageSize));state.page=Math.min(Math.max(1,state.page),pages);const start=(state.page-1)*pageSize;const page=list.slice(start,start+pageSize);const ps=stats(page);document.getElementById('thSummary').innerHTML=[kpi('Trades',String(s.count),pill(rangeLabel(state.range),'ok'),'&#9636;'),kpi('Total P&L',rs(s.pnl),pill(rangeLabel(state.range),s.pnl>=0?'ok':'bad'),'&#8377;'),kpi('Wins',String(s.wins),pill('Filtered','ok'),'&#8599;'),kpi('Losses',String(s.losses),pill('Filtered',s.losses?'bad':'ok'),'&#8600;'),kpi('Latest',list[0]?txt(list[0].date):'No session',pill(rangeLabel(state.range),list.length?'ok':'warn'),'&#9719;'),kpi(state.range==='monthly'?'Selected Month P&L':'This Month',rs(ms.pnl),pill(baseDate.slice(0,7),ms.pnl>=0?'ok':'bad'),'&#8377;')].join('');document.querySelectorAll('[data-th-range]').forEach(b=>b.classList.toggle('active',b.getAttribute('data-th-range')===state.range));document.getElementById('thType').value=state.type;document.getElementById('thSearch').value=state.q;document.getElementById('thCount').textContent=s.count+' '+(s.count===1?'record':'records');document.getElementById('thBody').innerHTML=page.length?page.map(rowHtml).join(''):'<tr class="ws-empty-row"><td colspan="9">'+empty('&#9636;','No futures trades found','No 10:30 futures trades match the selected range and filters.','')+'<div style="margin-top:10px;display:flex;gap:8px;justify-content:center"><button id="thLatestEmpty" class="btn" type="button">View Latest Session</button><button id="thResetEmpty" class="btn" type="button">Reset Filters</button></div></td></tr>';document.getElementById('thPageTotal').textContent='Showing '+(list.length?start+1:0)+'-'+Math.min(start+pageSize,list.length)+' of '+list.length;document.getElementById('thPageInfo').textContent='Page '+state.page+' of '+pages;document.getElementById('thPrev').disabled=state.page<=1;document.getElementById('thNext').disabled=state.page>=pages;document.getElementById('thTotals').innerHTML='<span>Page Trades '+ps.count+'</span><span>Page P&L '+rs(ps.pnl)+'</span><span>Page Wins '+ps.wins+'</span><span>Page Losses '+ps.losses+'</span><span>Selected P&L '+rs(s.pnl)+'</span><span>Win Rate '+s.winRate+'%</span>';document.getElementById('thInsights').innerHTML=[['Selected Range',rangeLabel(state.range)],['Best Trade',s.best==null?'No data':rs(s.best),s.best>=0?'ok':'bad'],['Worst Trade',s.worst==null?'No data':rs(s.worst),s.worst>=0?'ok':'bad'],['Average',s.count?rs(s.avg):'No data',s.avg>=0?'ok':'bad'],['Selected Range P&L',rs(s.pnl),s.pnl>=0?'ok':'bad'],['Selected Trades',String(s.count)],['Win Rate',s.winRate+'%'],['Monthly P&L',rs(ms.pnl),ms.pnl>=0?'ok':'bad'],['Data Scope','10:30 futures only','ok']].map(r=>'<div class="side-row"><label>'+r[0]+'</label><b class="'+(r[2]||'')+'">'+r[1]+'</b></div>').join('');const le=document.getElementById('thLatestEmpty');if(le)le.onclick=function(){state.range='latest-session';state.page=1;drawTradeHistory()};const re=document.getElementById('thResetEmpty');if(re)re.onclick=resetTradeHistory}
