...
function render(d){
  ...
  if(PAGE==='trade-history'){
    ...
    function rowHtml(t){const pnl=Number(t.pnl||0);const st=t.status||'Filled';return '<tr><td>'+txt(t.date||'')+'</td><td>'+txt(t.time||'')+'</td><td title="'+txt(t.symbol)+'">'+txt(t.symbol)+'</td><td>'+pill(t.side,t.side==='SELL'?'bad':'ok')+'</td><td class="right">'+txt(t.qty)+'</td><td class="right">'+txt(t.entry)+'</td><td class="right">'+txt(t.exit)+'</td><td class="right '+(pnl>=0?'ok':'bad')+'">'+rs(pnl)+'</td><td>'+pill(st,/reject|fail|error/i.test(st)?'bad':'ok')+'</td></tr>'}
