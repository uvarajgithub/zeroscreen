...
function computeAnalytics(trades) {
    ...
    const wkWins = wkTrades.filter((t) => tradePnlRs(t) > 0).length;
    const wkPnl = parseFloat(wkTrades.reduce((s, t) => s + tradePnlRs(t), 0).toFixed(1));
    // Monthly breakdown
    const monthMap = {};
    for (const t of trades) {
        if (!t.date)
    ...
    res.json({
        ...
        validations: {
            ...
            bot: {
                ...
                source: useDb ? "bot_state database mirror" : "bot-heartbeat.json",
...
function tradeOpsReadBotEnv() {
    try {
        const raw = fs_1.default.readFileSync(TRADEOPS_BOT_ENV, "utf8");
        const map = {};
        raw.split(/\r?\n/).forEach(line => {
...
function tradeOpsWriteBotEnvValue(key, value) {
    ...
    if (!found)
        next.push(`${key}=${value}`);
    fs_1.default.writeFileSync(TRADEOPS_BOT_ENV, next.join("\n") + "\n");
}
...
async function tradeOpsKiteJSON(pathname, init = {}) {
    const { apiKey, token } = await tradeOpsReadKiteCreds();
    if (!apiKey || !token)
        throw new Error("Broker token unavailable");
...
async function tradeOpsKiteText(pathname) {
    ...
    const text = await resp.text();
    if (!resp.ok)
        throw new Error(text || `Broker request failed (${resp.status})`);
    return text;
}
function tradeOpsCsvCell(row, idx) {
    return String(row[idx] || "").trim().replace(/^"|"$/g, "");
}
async function tradeOpsBankNiftyFuturesInstrument() {
    if (tradeOpsFuturesSymbolCache && Date.now() - tradeOpsFuturesSymbolCacheAt < 15 * 60 * 1000)
        return tradeOpsFuturesSymbolCache;
    const csv = await tradeOpsKiteText("/instruments/NFO");
    ...
    const daysToExpiry = (new Date(front.expiry).getTime() - Date.now()) / 86400000;
...
async function tradeOpsNextFuturesMargin(hb) {
    ...
    try {
        ...
        const margin = await tradeOpsKiteJSON("/margins/orders", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify(payload),
        });
        const row = Array.isArray(margin?.data) ? margin.data[0] : null;
        const total = tradeOpsMaybeNum(row?.total);
        const span = tradeOpsMaybeNum(row?.span);
        const exposure = tradeOpsMaybeNum(row?.exposure);
        return {
            ok: total !== null,
            symbol: fut.tradingsymbol,
            qty,
            lotSize: fut.lotSize,
            requiredMargin: Math.ceil(total ?? 0),
            span,
            exposure,
            source: "Zerodha order margin API",
        };
    }
    catch (e) {
        return {
            ...
            lotSize: 30,
            requiredMargin: 0,
            source: e?.message || "Margin calculation unavailable",
        };
    }
}
...
function tradeOpsNum(v, fallback = 0) {
    const n = Number(v);
    return Number.isFinite(n) ? n : fallback;
}
function tradeOpsMaybeNum(v) {
...
function tradeOpsSide(value) {
    const s = String(value || "").toUpperCase();
    if (s.includes("PE") || s.includes("SELL") || s === "SHORT")
        return "SELL";
    if (s.includes("CE") || s.includes("BUY") || s === "LONG")
        return "BUY";
    return s || "OPEN";
}
...
function tradeOpsSanitizeSymbol(value) {
    const raw = String(value || "").trim();
    if (!raw)
...
async function buildTradeOpsStatus() {
    ...
    if ((openTrade || Math.abs(unrealized) > 0) && Number.isFinite(netPnl))
        todayRangePnls.push(netPnl);
    ...
    const openPositions = openTrade ? (broker.positions || [])
        ...
        .filter((p) => tradeOpsIsFuturesTrade(p))
        .map((p) => ({
    ...
    const available = marginsSynced ? (availableRaw ?? 0) : null;
    const used = marginsSynced ? (usedRaw ?? 0) : null;
    const totalBalance = marginsSynced ? (totalBalanceRaw ?? (available ?? 0) + (used ?? 0)) : null;
    const exposurePct = marginsSynced && totalBalance > 0 ? Math.min(100, Math.max(0, (used ?? 0) / totalBalance * 100)) : null;
    ...
    const nextMargin = await tradeOpsNextFuturesMargin(hb);
    ...
    const tradeRows = strategyTrades.slice(-500).reverse().map(t => ({
        ...
        pnl: tradeOpsPnl(t),
        status: tradeOpsNum(t?.exitPrice) > 0 || tradeOpsNum(t?.premiumExit) > 0 ? "Filled" : "Pending",
    ...
    const candleEntryTime = String(hb1030Entry?.time || "").slice(0, 5);
    const candleDir = String(hb1030Entry?.dir || hb1030Exit?.dir || hb?.tt1030Dir || "").toUpperCase();
    ...
    return {
        ...
        validations: {
            broker: {
                ok: broker.ok,
                label: broker.ok ? "Broker verified" : "Broker unavailable",
                source: "Kite profile/margins REST checks",
                error: broker.error,
            },
            token: {
                ok: broker.tokenOK,
                label: broker.tokenOK ? "Token valid" : "Token refresh required",
                source: "Kite /user/profile HTTP 200",
                error: broker.tokenOK ? "" : broker.error,
            },
            bot: {
                ok: isAlive,
                label: isAlive ? "Bot heartbeat fresh" : "Bot heartbeat stale or missing",
                source: "bot-heartbeat.json",
                heartbeatAgeSec,
            },
            feed: {
                ok: feedFresh,
                label: feedFresh ? "Feed fresh" : "Feed waiting",
                source: "tt1030CandleLog heartbeat feed",
        ...
        execution: {
            ...
            marginSource: nextMargin.source,
            marginSymbol: nextMargin.symbol,
...
app.get("/api/tradeops/status", requireAdmin, async (_req, res) => {
    try {
        res.setHeader("Cache-Control", "no-store");
...
app.get("/api/tradeops/logs", requireAdmin, async (_req, res) => {
    try {
        res.setHeader("Cache-Control", "no-store");
...
app.post("/api/tradeops/mode", requireAdmin, async (req, res) => {
    try {
        ...
        fs_1.default.appendFileSync(auditPath, JSON.stringify({
        ...
        if (req.body?.restartBot === true) {
            (0, child_process_1.execSync)("pm2 reload trading-bot", { cwd: TRADEOPS_BOT_DIR, stdio: "pipe" });
            restarted = true;
        }
        res.json({
            ok: true,
...
function tradeOpsInitialWorkspaceHTML(page, status) {
    ...
    if (page === "orders" || page === "trade-history")
        return `${top}${kpis}${simpleRows(trades, ["Date", "Time", "Symbol", "Side", "Qty", "Entry", "Exit", "P&L", "Status"], (t) => `<tr><td>${h(t.date)}</td><td>${h(t.time)}</td><td>${h(t.symbol)}</td><td>${h(t.side)}</td><td>${h(t.qty)}</td><td>${h(t.entry)}</td><td>${h(t.exit)}</td><td>${h(rs(t.pnl || 0))}</td><td>${h(t.status)}</td></tr>`)}</section>`;
    if (page === "positions")
        return `${top}${kpis}${simpleRows(positions, ["Symbol", "Qty", "Avg", "LTP", "P&L", "Status"], (p) => `<tr><td>${h(p.symbol)}</td><td>${h(p.qty)}</td><td>${h(p.avg)}</td><td>${h(p.ltp)}</td><td>${h(rs(p.pnl || 0))}</td><td>${h(p.status)}</td></tr>`)}</section>`;
    if (page === "candle-logs")
...
.tradeops-loader{position:fixed;inset:0;z-index:80;display:grid;place-items:center;background:linear-gradient(180deg,#06172d,#02152c);transition:opacity .18s ease,visibility .18s ease}
...
function render(d){
  ...
  const tradeList=Array.isArray(d.trades)?d.trades:[];
  ...
  set('pnlSource',(d.pnl&&d.pnl.source?d.pnl.source:(d.broker.connected?'Broker':'Bot'))+' / refresh '+Math.round(((d.pnl&&d.pnl.refreshMs)||5000)/1000)+'s');
  ...
  ['realized','unrealized','dayHigh','dayLow','openPnl','closedPnl'].forEach(id=>{const e=document.getElementById(id);if(e)e.className=String(e.textContent||'').startsWith('-')?'bad':(String(e.textContent||'').includes('Not')?'pending':'ok')});
  ['charges','netAfterCharges'].forEach(id=>{const e=document.getElementById(id);if(e)e.className=String(e.textContent||'').includes('Not')?'pending':(String(e.textContent||'').startsWith('-')?'bad':'ok')});
  ...
  set('ltp',c?'LTP '+c.close.toFixed(2):'LTP unavailable');
  set('change',c&&c.open&&!c.closeOnly?((c.close-c.open)>=0?'+':'')+(c.close-c.open).toFixed(2)+' ('+(((c.close-c.open)/c.open)*100).toFixed(2)+'%)':'');
  ...
  document.getElementById('ordersBody').innerHTML=hasOrders?d.trades.slice(0,5).map(t=>{const st=/reject|fail|error/i.test(t.status||t.note||'')?'Rejected':/pending|open|sent/i.test(t.status||'')?'Pending':'Filled';const pnl=Number(t.pnl||0);return '<tr><td>'+t.time+'</td><td title="'+t.symbol+'">'+t.symbol+'</td><td class="'+(t.side==='SELL'?'bad':'ok')+'">'+t.side+'</td><td>'+t.qty+'</td><td>'+t.exit.toFixed(2)+'</td><td class="'+cls(pnl)+'">'+rs(pnl)+'</td><td><span class="badge">'+st+'</span></td></tr>'}).join(''):'';
  renderLogs(d.logs||[]);
  ...
  if(PAGE==='trade-history'){
    ...
    function dateKey(t){return tradeDate(t)}
    ...
    function rowHtml(t){const pnl=Number(t.pnl||0);const st=t.status||'Filled';const sym=rowSymbol(t);return '<tr><td>'+txt(t.date||'')+'</td><td>'+txt(t.time||'')+'</td><td title="'+txt(sym)+'">'+txt(sym)+'</td><td>'+pill(t.side,t.side==='SELL'?'bad':'ok')+'</td><td class="right">'+txt(t.qty)+'</td><td class="right">'+txt(t.entry)+'</td><td class="right">'+txt(t.exit)+'</td><td class="right '+(pnl>=0?'ok':'bad')+'">'+rs(pnl)+'</td><td>'+pill(st,/reject|fail|error/i.test(st)?'bad':'ok')+'</td></tr>'}
    const scope='<section class="ws-card scope-card"><div class="ws-card-b"><div class="scope-title">Data Scope</div><div class="scope-text">Showing 10:30 futures-only BANKNIFTY live/session trade records. Backtest, options, shadow, and index-only rows are excluded.</div><div class="scope-badges"><span class="ws-pill ok">10:30 futures only</span><span class="ws-pill ok">Live/session data</span><span class="ws-pill warn">Backtest excluded</span></div></div></section>';
    root.innerHTML=header('Trade History','Analyze completed 10:30 futures trades by date, symbol, status, and outcome.')+scope+'<div id="thSummary" class="ws-summary"></div><section class="ws-card"><div class="ws-card-b"><div class="filter-bar trade-history-filters" data-skip-workspace-wire="1"><button class="seg active" data-th-range="today" type="button">Today</button><button class="seg" data-th-range="latest-session" type="button">Latest Session</button><button class="seg" data-th-range="weekly" type="button">Weekly</button><button class="seg" data-th-range="monthly" type="button">Monthly</button><select id="thType"><option value="all">All Trades</option><option value="winners">Winners</option><option value="losers">Losers</option><option value="buy">Buy</option><option value="sell">Sell</option><option value="closed">Closed</option><option value="rejected">Rejected</option></select><input id="thSearch" placeholder="Search..."><button id="thApply" class="btn primary" type="button">Apply Filter</button><button id="thReset" class="btn" type="button">Reset</button></div></div></section><div class="ws-grid"><div class="ws-main"><section class="ws-card"><div class="ws-card-h"><span id="thTableTitle">Trade History - Today</span><span id="thCount" class="muted">0 records</span></div><div class="ws-card-b"><div class="ws-table-wrap"><table class="ws-table"><thead><tr><th>Date</th><th>Time</th><th>Symbol</th><th>Side</th><th class="right">Qty</th><th class="right">Entry</th><th class="right">Exit</th><th class="right">P&L</th><th>Status</th></tr></thead><tbody id="thBody"></tbody></table></div><div id="thTotals" class="workflow-line" style="margin-top:10px"></div><div class="ws-table-footer"><span id="thPageTotal">Showing 0-0 of 0</span><div class="ws-pager"><button id="thPrev" class="btn" type="button">Prev</button><span id="thPageInfo">Page 1 of 1</span><button id="thNext" class="btn" type="button">Next</button></div></div></div></section></div><aside class="ws-side"><section class="ws-card"><div class="ws-card-h"><span>Trade Insights</span></div><div id="thInsights" class="ws-card-b side-list"></div></section></aside></div></section>';
    const state={range:'today',type:'all',q:'',page:1};
    const pageSize=10;
    function drawTradeHistory(){const list=filteredTrades(state);const s=stats(list);const monthList=allTrades.filter(t=>inRange(t,'monthly'));const ms=stats(monthList);const pages=Math.max(1,Math.ceil(list.length/pageSize));state.page=Math.min(Math.max(1,state.page),pages);const start=(state.page-1)*pageSize;const page=list.slice(start,start+pageSize);const ps=stats(page);const label=rangeLabel(state.range);document.getElementById('thSummary').innerHTML=[kpi('Trades',String(s.count),pill(label,'ok'),'&#9636;'),kpi('Total P&L',rs(s.pnl),pill(label,s.pnl>=0?'ok':'bad'),'&#8377;'),kpi('Wins',String(s.wins),pill(label,'ok'),'&#8599;'),kpi('Losses',String(s.losses),pill(label,s.losses?'bad':'ok'),'&#8600;'),kpi('Latest',list[0]?txt(list[0].date):'No session',pill(label,list.length?'ok':'warn'),'&#9719;'),kpi(state.range==='monthly'?'Selected Month P&L':'This Month P&L',rs(ms.pnl),pill(baseDate.slice(0,7),ms.pnl>=0?'ok':'bad'),'&#8377;')].join('');document.querySelectorAll('[data-th-range]').forEach(b=>b.classList.toggle('active',b.getAttribute('data-th-range')===state.range));document.getElementById('thTableTitle').textContent='Trade History - '+label;document.getElementById('thType').value=state.type;document.getElementById('thSearch').value=state.q;document.getElementById('thCount').textContent=s.count+' '+(s.count===1?'record':'records');document.getElementById('thBody').innerHTML=page.length?page.map(rowHtml).join(''):'<tr class="ws-empty-row"><td colspan="9">'+empty('&#9636;','No trades found','No 10:30 futures trades found for the selected filter.','')+'<div style="margin-top:10px;display:flex;gap:8px;justify-content:center"><button id="thLatestEmpty" class="btn" type="button">View Latest Session</button><button id="thResetEmpty" class="btn" type="button">Reset Filters</button></div></td></tr>';document.getElementById('thPageTotal').textContent='Showing '+(list.length?start+1:0)+'-'+Math.min(start+pageSize,list.length)+' of '+list.length;document.getElementById('thPageInfo').textContent='Page '+state.page+' of '+pages;document.getElementById('thPrev').disabled=state.page<=1;document.getElementById('thNext').disabled=state.page>=pages;document.getElementById('thTotals').innerHTML='<span>Page Trades '+ps.count+'</span><span>Page P&L '+rs(ps.pnl)+'</span><span>Page Wins '+ps.wins+'</span><span>Page Losses '+ps.losses+'</span><span>Selected Trades '+s.count+'</span><span>Selected P&L '+rs(s.pnl)+'</span><span>Selected Win Rate '+s.winRate+'%</span>';document.getElementById('thInsights').innerHTML=[['Selected Range',label],['Best Trade',s.best==null?'No data':rs(s.best),s.best>=0?'ok':'bad'],['Worst Trade',s.worst==null?'No data':rs(s.worst),s.worst>=0?'ok':'bad'],['Average',s.count?rs(s.avg):'No data',s.avg>=0?'ok':'bad'],['Selected Range P&L',rs(s.pnl),s.pnl>=0?'ok':'bad'],['Selected Trades',String(s.count)],['Win Rate',s.winRate+'%'],['Monthly P&L',rs(ms.pnl),ms.pnl>=0?'ok':'bad'],['Data Scope','10:30 futures only','ok']].map(r=>'<div class="side-row"><label>'+r[0]+'</label><b class="'+(r[2]||'')+'">'+r[1]+'</b></div>').join('');const le=document.getElementById('thLatestEmpty');if(le)le.onclick=function(){state.range='latest-session';state.page=1;drawTradeHistory()};const re=document.getElementById('thResetEmpty');if(re)re.onclick=resetTradeHistory}
    function applyTradeHistory(){state.type=document.getElementById('thType').value;state.q=document.getElementById('thSearch').value;state.page=1;drawTradeHistory()}
    function resetTradeHistory(){state.range='today';state.type='all';state.q='';state.page=1;drawTradeHistory()}
    ...
    document.getElementById('thApply').onclick=applyTradeHistory;
    document.getElementById('thReset').onclick=resetTradeHistory;
    document.getElementById('thSearch').onkeydown=function(e){if(e.key==='Enter')applyTradeHistory()};
    document.getElementById('thPrev').onclick=function(){state.page--;drawTradeHistory()};
    document.getElementById('thNext').onclick=function(){state.page++;drawTradeHistory()};
    drawTradeHistory();
    return;
  }
  if(PAGE==='candle-logs'){
    function istParts(){const parts=new Intl.DateTimeFormat('en-CA',{timeZone:'Asia/Kolkata',year:'numeric',month:'2-digit',day:'2-digit',hour:'2-digit',minute:'2-digit',hour12:false}).formatToParts(new Date()).reduce((a,p)=>(a[p.type]=p.value,a),{});return {date:parts.year+'-'+parts.month+'-'+parts.day,mins:Number(parts.hour||0)*60+Number(parts.minute||0)}}
    function minFromTime(v){const m=String(v||'').match(/(\d{1,2}):(\d{2})/);return m?Number(m[1])*60+Number(m[2]):null}
    function inferTf(list){const mins=list.map(c=>minFromTime(c.time)).filter(x=>x!=null).sort((a,b)=>a-b);const diffs=[];for(let i=1;i<mins.length;i++){const d=mins[i]-mins[i-1];if(d>0&&d<=60)diffs.push(d)}const d=diffs[0]||15;return d<=5?'5m':'15m'}
    function readableState(c){const raw=String(c.status||'').toLowerCase().replace(/\s+/g,'_');const note=String(c.note||'').toLowerCase();const s=raw||note;if(/re[-_ ]?exit/.test(s))return ['Re-exit','bad'];if(/exit_eod|eod/.test(s))return ['EOD Exit','bad'];if(/sl[_ -]?hit|stop/.test(s))return ['SL Hit','bad'];if(/re[-_ ]?entry/.test(s))return ['Re-entry','ok'];if(/entry/.test(s))return ['Entry','ok'];if(/trail/.test(s))return ['Trail','warn'];if(/hold|in_trade/.test(s))return ['Hold',''];if(/monitor|watch/.test(s))return ['Watching',''];if(/exit/.test(s))return ['Exit','bad'];return [c.status?titleCase(String(c.status).replace(/_/g,'-')):'Watching','']}
    function isEntryState(label){return /Entry|Re-entry/.test(label)}
    function isExitState(label){return /Exit|Re-exit|SL Hit|EOD Exit/.test(label)}
    function sideBadge(side){const s=String(side||'--').toUpperCase();const k=/PE|SELL|SHORT/.test(s)?'bad':/CE|BUY|LONG/.test(s)?'ok':'';return pill(s||'--',k)}
    ...
    const entryCandle=viewCandles.find(c=>isEntryState(readableState(c)[0])&&c.entry!=null)||viewCandles.find(c=>c.entry!=null);
    const sideCandle=viewCandles.find(c=>c.dir&&c.dir!=='--')||entryCandle;
...
function wireWorkspaceControls(){
  ...
  document.querySelectorAll('.filter-bar').forEach(function(bar){
    if(bar.dataset&&bar.dataset.skipWorkspaceWire==='1') return;
...
async function setTradeOpsMode(mode){
  ...
  const restartBot=confirm('Save 10:30 futures mode as '+mode+' and reload trading-bot so the running process uses it?');
  const body={mode:mode,restartBot:restartBot};
  if(mode==='LIVE'){
    if(!confirm('10:30 futures LIVE places real Zerodha futures orders using real account cash. Continue?'))return;
    const phrase=prompt('Type ENABLE LIVE TRADING to confirm real-money mode');
    body.phrase=phrase||'';
    body.understandRealMoney=true;
  }
  if(out){out.style.display='block';out.textContent='Applying '+mode+'...';}
  try{
...
app.get("/tradeops", requireAdmin, async (_req, res) => {
    res.setHeader("Cache-Control", "no-store");
    const initialStatus = await buildTradeOpsStatus().catch((e) => ({ ok: false, error: e?.message || "Status unavailable" }));
