...
exports.sendTelegramDailyReport = sendTelegramDailyReport;
exports.generateDailyReportContent = generateDailyReportContent;
exports.executeDailyEodReportDispatch = executeDailyEodReportDispatch;
exports.startDailyEodScheduler = startDailyEodScheduler;
...
const SENT_TRACKER_FILE = path_1.default.join(process.cwd(), "eod-report-tracker.json");
...
async function sendTelegramDailyReport(text) {
    ...
    return new Promise((resolve) => {
        try {
            ...
            const req = https_1.default.request(`https://api.telegram.org/bot${TELEGRAM_TOKEN}/sendMessage`, {
                method: "POST",
                headers: {
...
function generateDailyReportContent() {
    const payload = (0, shadowMonitor_1.buildShadowMonitorPayload)("BANKNIFTY", "tt1030", "FUTURES");
    const { ymd, formatted } = getISTDateParts();
    const tiles = payload.consolidated?.tiles || [];
    const lifecycle = (0, shadowMonitor_1.getFuturesContractLifecycle)("BANKNIFTY", ymd);
...
async function executeDailyEodReportDispatch(force = false) {
    ...
    if (!force && fs_1.default.existsSync(SENT_TRACKER_FILE)) {
        try {
            const data = JSON.parse(fs_1.default.readFileSync(SENT_TRACKER_FILE, "utf8"));
    ...
    try {
        fs_1.default.writeFileSync(SENT_TRACKER_FILE, JSON.stringify({ lastSentDate: ymd, sentAt: new Date().toISOString(), summary }, null, 2));
    }
    catch { }
    return { ok: tgOk, message: `Report dispatched for ${ymd}. Net P&L: ${formatShortMoney(summary.totalPnl)}` };
}
function startDailyEodScheduler() {
    ...
    setInterval(async () => {
        try {
            ...
            if (dayOfWeek >= 1 && dayOfWeek <= 5 && hour === 15 && minute >= 35 && minute <= 45) {
                ...
                if (fs_1.default.existsSync(SENT_TRACKER_FILE)) {
                    try {
                        const data = JSON.parse(fs_1.default.readFileSync(SENT_TRACKER_FILE, "utf8"));
                        if (data.lastSentDate === ymd)
                            alreadySent = true;
