...
exports.getFuturesContractLifecycle = getFuturesContractLifecycle;
...
function underlyingId(value) {
    return String(value || "").toUpperCase() === "NIFTY" ? "NIFTY" : "BANKNIFTY";
}
function displayStrategyName(strategy, underlying) {
    return `${strategy.name.replace(/\s*\(BANKNIFTY\)\s*$/i, "")} (${underlying})`;
}
function runtimeFiles(strategy, underlying) {
...
function strategiesForUnderlying(underlying) {
    return STRATEGIES;
}
function strategyAvailableForUnderlying(strategy, underlying) {
    if (underlying === "BANKNIFTY")
        return true;
    const stateRoot = readJson(UNDERLYINGS.NIFTY.stateFile, {});
    const heartbeat = readJson(UNDERLYINGS.NIFTY.heartbeatFile, {});
...
function readJson(file, fallback) {
    try {
        let filePath = path_1.default.join(BOT_DIR, file);
        if (!fs_1.default.existsSync(filePath)) {
            const localPath = path_1.default.join(process.cwd(), file);
            if (fs_1.default.existsSync(localPath)) {
                filePath = localPath;
            }
            else {
                const rootPath = path_1.default.resolve(__dirname, "..", file);
        ...
        const mtimeMs = fs_1.default.statSync(filePath).mtimeMs;
        const cached = jsonFileCache.get(filePath);
        if (cached && cached.mtimeMs === mtimeMs)
            return cached.value;
        const value = parseFirstJsonValue(fs_1.default.readFileSync(filePath, "utf8"));
...
function genericShadowFields(state, hb, instrument, defaultQuantity) {
...
function strategyFields(strategy, hb, state, instrument, underlying = "BANKNIFTY") {
    if (underlying === "NIFTY")
        return genericShadowFields(state, hb, instrument, UNDERLYINGS.NIFTY.quantity);
    const isOptions = instrument === "OPTIONS";
    ...
    if (strategy.prefix === "bodyHoldS1" || strategy.prefix === "bodyHoldS2") {
        ...
        const rawTrades = (Array.isArray(ledger) ? ledger : []).filter((row) => String(row?.type || "").toUpperCase() === expectedType);
        const todayRows = rawTrades.filter((row) => dateKey(row?.date) === todayIST());
        const quantity = num(hb.qty) ?? 30;
        const realized = num(hb[isOptions ? `${hbPrefix}OptionsRealizedPnL` : `${hbPrefix}FuturesRealizedPnL`])
            ...
            ?? todayRows.reduce((sum, row) => sum + (num(row?.pnlRs) ?? 0), 0);
        ...
        const candleLog = processedCandleKeys.map((key, idx) => {
        ...
        const phase = inTrade
            ? "IN TRADE"
            : todayRows.length || processedCandleKeys.some((key) => String(key).endsWith("|15:15"))
                ? "DONE"
                : hb.status;
        ...
        return {
            ...
            wins: todayRows.filter((row) => (num(row?.pnlRs) ?? 0) > 0).length,
            losses: todayRows.filter((row) => (num(row?.pnlRs) ?? 0) < 0).length,
            direction,
    ...
    const realized = num(hb[isOptions ? `${fieldPrefix}OptClosedPnL` : `${fieldPrefix}ClosedPnL`])
        ?? num(isOptions ? persisted.optDayRs : persisted.dayRs)
        ?? 0;
    const livePrice = num(isOptions
        ? (hb[`${fieldPrefix}OptionLive`] ?? persisted.optLivePrem ?? hb.livePremium)
        : (hb[`${fieldPrefix}FuturesLive`] ?? hb.drishtiFuturesLive ?? hb.livePrice ?? persisted.live));
    const entryPrice = num(isOptions
    ...
    const total = inTrade
        ? (heartbeatTotal ?? (realized + computedUnrealized))
        : realized;
...
function recentLiveMonthKeys(tradeDate = todayIST()) {
...
function normalizedBacktestDays(strategy, instrument) {
    if (!strategy.backtestFile)
        return [];
    const data = readJson(strategy.backtestFile, null);
    if (!data)
...
function shadowHistory(strategy, instrument, includeBacktest = true, underlying = "BANKNIFTY") {
    ...
    if (underlying === "NIFTY") {
        ...
        const rows = (Array.isArray(ledger) ? ledger : []).filter((row) => {
            if (String(row?.type || "").toUpperCase() !== ledgerType)
                return false;
        ...
        for (const [date, dayRows] of grouped) {
            ...
            const capitalDeployed = dayRows.reduce((sum, row) => {
                const entry = num(instrument === "OPTIONS" ? row?.premiumEntry ?? row?.premIn : row?.entryPrice ?? row?.entry);
                const quantity = num(row?.qty ?? row?.quantity) ?? 30;
...
function runtimeEvidence(processName = "trading-bot") {
    ...
    try {
        ...
        const tradingBot = Array.isArray(rows)
            ? processNames.map(name => rows.find((row) => row?.name === name)).find(Boolean)
            : null;
        if (tradingBot) {
            processInfo = {
...
const STRATEGY_TRIGGER_MINUTES = {
    drishti: 565,
    "drishti-v2": 570,
    tt0945: 585,
    tt1000: 600,
...
function scheduledEvaluationMissed(strategyId, fields) {
...
function systemHealth(strategy, hb, fields, candles, heartbeatAgeSec, externalHealth, underlying) {
    ...
    const botDirectoryAvailable = fs_1.default.existsSync(BOT_DIR);
    ...
    const heartbeatFile = path_1.default.join(BOT_DIR, underlying === "NIFTY" ? UNDERLYINGS.NIFTY.heartbeatFile : strategy.heartbeatFile || "bot-heartbeat.json");
...
function getFuturesContractLifecycle(underlying = "BANKNIFTY", asOfDate = new Date()) {
    ...
    let prevCycleYear = prevYear;
    let prevCycleMonthIdx = prevMonthIdx - 1;
    if (prevCycleMonthIdx < 0) {
        prevCycleMonthIdx = 11;
        prevCycleYear -= 1;
    }
    const prevCycleStartExpiry = getMonthlyExpiry(prevCycleYear, prevCycleMonthIdx);
    const prevCycleStartDate = new Date(prevCycleStartExpiry.getTime() + 24 * 3600000);
    const prevCycleExpiryDate = prevExpiry;
    const prevYy = String(prevYear).slice(2);
    const prevSymbol = `${u}${prevYy}${monthNames[prevMonthIdx]}FUT`;
    return {
        underlying: u,
        current: {
            symbol: activeSymbol,
        ...
        previous: {
            symbol: prevSymbol,
            startDate: formatYmd(prevCycleStartDate),
            startDateDisplay: formatDate(prevCycleStartDate),
            expiryDate: formatYmd(prevCycleExpiryDate),
            expiryDateDisplay: formatDate(prevCycleExpiryDate),
            monthKey: `${prevYear}-${String(prevMonthIdx + 1).padStart(2, "0")}`,
        },
        asOf: istTime.toISOString(),
    };
}
function bankNiftyMovement() {
    ...
    const current = num(heartbeat?.livePrice
        ?? heartbeat?.tt1030Live
        ?? heartbeat?.tt1030ShadowLive
        ?? heartbeat?.normalBreakoutShadowLive) ?? candleClose;
    const highs = rows.map(row => num(row?.high ?? row?.h)).filter((value) => value !== null);
    const lows = rows.map(row => num(row?.low ?? row?.l)).filter((value) => value !== null);
    if (current !== null) {
        highs.push(current);
        lows.push(current);
    }
...
function underlyingMovement(underlying) {
    if (underlying === "BANKNIFTY")
        return bankNiftyMovement();
    const heartbeat = readJson(UNDERLYINGS.NIFTY.heartbeatFile, {});
...
function capturedPoints(pnl, quantity, traded) {
    if (!traded || pnl === null || quantity === null || quantity <= 0)
        return null;
    return pnl / quantity;
}
...
function consolidatedShadowSummary(externalHealth = {}, underlying = "BANKNIFTY") {
    const today = todayIST();
    const market = marketStatus();
    const availableStrategies = strategiesForUnderlying(underlying);
    const latestHistoryDate = availableStrategies.flatMap(strategy => (strategy.instruments || ["FUTURES", "OPTIONS"]).flatMap(instrument => (shadowHistory(strategy, instrument, false, underlying).days || [])
        .filter((row) => (num(row?.trades) ?? 0) > 0)
        .map((row) => String(row?.date || row?.period || ""))
        .filter(date => date && date <= today))).sort().reverse()[0] || "";
        ...
        for (const instrument of strategy.instruments || ["FUTURES", "OPTIONS"]) {
            ...
            const todayHistory = (() => {
                ...
                return Array.isArray(history?.days)
                    ? history.days.find((row) => row?.date === tradeDate || row?.period === tradeDate)
                    : null;
            })();
            ...
            const lifecycle = getFuturesContractLifecycle(underlying, tradeDate);
            const monthlyHistory = shadowHistory(strategy, instrument, false, underlying);
            const days = Array.isArray(monthlyHistory?.days) ? monthlyHistory.days : [];
            // 1. Current Contract Cycle Days: strictly within active contract start and expiry dates (auto-resets on new contract start)
            const currentContractDays = days.filter((d) => {
                const dateStr = String(d.date || d.period || "");
                return dateStr >= lifecycle.current.startDate && dateStr <= lifecycle.current.expiryDate;
            });
            const historyCurrentPnl = currentContractDays.reduce((sum, d) => sum + (num(d.pnl) ?? 0), 0);
            const liveDelta = fields.inTrade ? (pnl - (historyPnl ?? 0)) : (hasTradePnl && historyPnl === null ? pnl : 0);
            const currentContractPnl = Math.round(historyCurrentPnl + liveDelta);
            ...
            if (prevCycleDays.length === 0 || lastMonthPnl === 0 || VARIANT_BT_JULY[strategy.id]) {
                ...
                if (variantData) {
                    ...
                    if (bt?.months && Array.isArray(bt.months) && bt.months.length > 0) {
                        const prevMonthObj = bt.months.find((m) => m.period === lifecycle.previous?.monthKey) || bt.months[bt.months.length - 1];
                        if (prevMonthObj) {
            ...
            tiles.push({
                ...
                capitalDeployed,
                currentContractPnl,
                currentContractReturnPct,
                currentContractTrades,
                lastMonthPnl,
                lastMonthReturnPct,
                lastMonthTrades,
                monthPnl: currentContractPnl,
                monthReturnPct: currentContractReturnPct,
                monthTrades: currentContractTrades,
                trades: tradeCount,
...
function performanceOverview(hb, state, tradeDate, underlying = "BANKNIFTY") {
    const availableStrategies = strategiesForUnderlying(underlying);
    ...
    const liveTodayFallback = [];
    const dailyCandidates = [];
    const performanceRow = (strategy, instrument, periodRow, fallbackCapital) => {
        ...
        for (const instrument of supportedInstruments) {
            ...
            if (Array.isArray(liveHistory.days)) {
                for (const day of liveHistory.days) {
                    ...
                    if (!date || date > tradeDate || num(day?.pnl) === null || (num(day?.trades) ?? 0) <= 0)
                        continue;
    ...
    const summarize = (periodRows, label, source) => {
        ...
        const worst = (instrument) => periodRows
            .filter(row => row.instrument === instrument)
...
function buildShadowMonitorPayload(strategyId = "", instrumentValue = "", externalHealth = {}, underlyingValue = "BANKNIFTY") {
...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box}.sm-page{background:var(--sm-bg);color:var(--sm-ink);min-height:calc(100vh - 104px);padding:16px 18px 26px;overflow-x:hidden}.sm-shell{max-width:1600px;margin:0 auto}.sm-head{display:flex;align-items:flex-start;justify-content:space-between;gap:16px;margin-bottom:12px}.sm-title-row{display:flex;align-items:center;gap:10px;flex-wrap:wrap}.sm-title{font-size:28px;line-height:1.2;margin:0;font-weight:780}.sm-shadow-badge{border:1px solid #c4b5fd;background:#f5f3ff;color:#6d28d9;border-radius:6px;padding:5px 9px;font-size:12px;font-weight:800}.sm-sub{font-size:13px;color:var(--sm-muted);margin:5px 0 0}.sm-head-actions{display:flex;align-items:center;gap:10px}.sm-market,.sm-refresh,.sm-history-btn,.sm-link-btn,.sm-resume{height:38px;border-radius:7px;border:1px solid var(--sm-line);background:#fff;padding:0 14px;font-size:13px;font-weight:750;color:var(--sm-ink);display:inline-flex;align-items:center;gap:7px;cursor:pointer;white-space:nowrap}.sm-market.open{color:var(--sm-green);border-color:#9fdfa9;background:#f0fdf4}.sm-market.closed{color:var(--sm-muted)}.sm-dot{width:8px;height:8px;border-radius:50%;background:currentColor;display:inline-block}.sm-icon{width:18px;height:18px;fill:none;stroke:currentColor;stroke-width:2.2;stroke-linecap:round;stroke-linejoin:round}.sm-control{display:grid;grid-template-columns:minmax(270px,1.5fr) 252px 210px 200px;gap:12px;align-items:end}.sm-control-card{padding:13px 14px;margin-bottom:10px}.sm-field label{display:block;font-size:12px;color:var(--sm-muted);font-weight:700;margin:0 0 6px}.sm-select,.sm-today{width:100%;height:40px;border:1px solid var(--sm-line);border-radius:7px;background:#fff;color:var(--sm-ink);padding:0 12px;font-size:14px;outline:none;display:flex;align-items:center}.sm-segment{display:grid;grid-template-columns:1fr 1fr;height:40px;border:1px solid var(--sm-line);border-radius:7px;overflow:hidden;background:#fff}.sm-segment button{border:0;background:#fff;color:#334155;font-size:13px;font-weight:750;cursor:pointer}.sm-segment button.active{background:var(--sm-blue);color:#fff;box-shadow:none}.sm-mode-box{height:40px;border:1px solid #a7e0b2;border-radius:7px;background:#effdf3;color:var(--sm-green);padding:5px 10px;display:flex;flex-direction:column;justify-content:center}.sm-mode-box b{font-size:12px}.sm-mode-box span{font-size:10px;color:#4b7d5b}.sm-refresh-meta{text-align:right;font-size:12px;color:var(--sm-muted);padding:0}.sm-health{display:grid;grid-template-columns:.9fr .9fr 1fr 1.8fr;gap:10px;margin-bottom:10px}.sm-card{background:var(--sm-card);border:1px solid var(--sm-line);border-radius:8px;box-shadow:0 1px 2px rgba(15,23,42,.04);min-width:0}.sm-health-card{position:relative;min-height:76px;padding:10px 12px;display:flex;align-items:center;gap:10px}.sm-health-card:not(:last-child)::after{content:"";position:absolute;width:10px;height:2px;right:-11px;top:50%;background:#b7e4c5;z-index:2}.sm-health-card.good-step{border-color:#a7e0b2;background:#f6fff8}.sm-health-icon{width:36px;height:36px;border-radius:50%;display:grid;place-items:center;background:#eef5ff;color:var(--sm-blue);flex:0 0 auto}.sm-health-icon.good{background:#dcfce7;color:var(--sm-green)}.sm-health-icon.warn{background:#fff7ed;color:var(--sm-amber)}.sm-health-copy{min-width:0;flex:1}.sm-card-label{font-size:11px;color:#475569;margin-bottom:3px}.sm-health-value{font-size:15px;font-weight:800;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.sm-health-note{font-size:10px;color:var(--sm-muted);margin-top:2px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.sm-backtest{display:grid;grid-template-columns:auto repeat(6,minmax(0,1fr));gap:0;align-items:center;width:100%}.sm-bt-cell{padding:0 8px;border-left:1px solid var(--sm-line);min-width:0}.sm-bt-cell:first-of-type{border-left:0}.sm-bt-cell span{display:block;font-size:10px;color:var(--sm-muted);white-space:nowrap}.sm-bt-cell b{display:block;font-size:12px;margin-top:3px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.sm-source{font-size:10px;text-transform:uppercase;color:var(--sm-muted);font-weight:800;letter-spacing:.04em}.sm-main-grid{display:grid;grid-template-columns:1.45fr .95fr;gap:12px;margin-bottom:12px}.sm-pnl{padding:16px 18px;min-height:262px;border-color:#cbd5e1}.sm-pnl.profit{background:#eefbf4;border-color:#8dd9aa}.sm-pnl.loss{background:#fff1f1;border-color:#f3a4a4}.sm-card-head{display:flex;align-items:center;justify-content:space-between;gap:12px;margin-bottom:11px}.sm-card-head h2{font-size:14px;margin:0;font-weight:800;text-transform:uppercase}.sm-live-chip,.sm-type-chip,.sm-status{height:25px;border-radius:4px;border:1px solid #a7e0b2;background:#effdf3;color:var(--sm-green);padding:0 8px;display:inline-flex;align-items:center;gap:6px;font-size:10px;font-weight:800}.sm-type-chip{border-color:#b8d2ff;background:#eff5ff;color:var(--sm-blue)}.sm-pnl-value{text-align:center;font-size:44px;font-weight:800;line-height:1.05;color:#334155;margin:12px 0 4px}.sm-pnl-sub{text-align:center;font-size:12px;color:var(--sm-muted);margin-bottom:12px}.sm-pnl-state{display:inline-flex;margin-left:6px;padding:3px 7px;border-radius:4px;background:#dcfce7;color:var(--sm-green);font-size:10px;font-weight:800}.sm-pnl.loss .sm-pnl-state{background:#fee2e2;color:var(--sm-red)}.sm-pnl.profit .sm-pnl-value{color:var(--sm-green)}.sm-pnl.loss .sm-pnl-value{color:var(--sm-red)}.sm-metrics{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:8px}.sm-metric{padding:9px 10px;border:1px solid rgba(100,116,139,.16);background:rgba(255,255,255,.78);border-radius:5px;min-width:0}.sm-metric span{display:block;font-size:11px;color:var(--sm-muted);line-height:1.25}.sm-metric b{display:block;font-size:14px;margin-top:4px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.sm-card-foot{font-size:12px;color:var(--sm-muted);border-top:1px solid rgba(100,116,139,.18);margin-top:11px;padding-top:9px}.sm-snapshot{padding:15px 16px;min-height:262px}.sm-snapshot-grid{display:grid;grid-template-columns:1fr 1fr}.sm-kv{min-height:31px;border-top:1px dashed var(--sm-line);padding:7px 0;display:grid;grid-template-columns:1fr auto;gap:10px;font-size:12px}.sm-kv:nth-child(odd){padding-right:16px}.sm-kv:nth-child(even){padding-left:16px;border-left:1px solid var(--sm-line)}.sm-kv span{color:var(--sm-muted)}.sm-kv b{text-align:right;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.sm-empty{height:194px;display:grid;place-items:center;text-align:center;color:var(--sm-muted);padding:20px;font-size:12px}.sm-empty b{display:block;color:var(--sm-ink);font-size:15px;margin-bottom:6px}.sm-table-card,.sm-log-card{display:flex;flex-direction:column}.sm-trade-card{height:360px;margin-bottom:12px}.sm-detail-grid{display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:12px}.sm-detail-card{height:330px}.sm-console-card{height:280px}.sm-card-pad{padding:13px 14px 11px}.sm-table-wrap{overflow:auto;min-height:0;flex:1;border-top:1px solid var(--sm-line)}.sm-table{width:100%;border-collapse:collapse;min-width:900px}.sm-table.candles{min-width:900px}.sm-table th{height:34px;text-align:left;padding:0 9px;color:var(--sm-muted);font-size:11px;font-weight:760;background:#f8fafc;position:sticky;top:0;z-index:1}.sm-table td{height:38px;border-top:1px solid #edf1f6;padding:0 9px;font-size:12px;white-space:nowrap}.sm-table td.note{max-width:260px;overflow:hidden;text-overflow:ellipsis}.sm-table tr.candle-entry td{background:#f0fdf4}.sm-table tr.candle-exit td,.sm-table tr.candle-sl td{background:#fff1f2}.sm-table tr.candle-trail td{background:#fffbeb}.sm-table tr.candle-watch td{background:#f8fafc}.sm-state{display:inline-flex;border-radius:5px;padding:4px 7px;font-size:10px;font-weight:800;background:#e2e8f0;color:#475569}.sm-state.entry,.sm-state.re-entry{background:#dcfce7;color:#15803d}.sm-state.exit,.sm-state.eod-exit,.sm-state.sl-hit,.sm-state.re-exit{background:#fee2e2;color:#dc2626}.sm-state.trail{background:#fef3c7;color:#b45309}.sm-state.watching,.sm-state.hold{background:#dbeafe;color:#1d4ed8}.sm-side{display:inline-flex;min-width:34px;justify-content:center;border-radius:999px;padding:4px 8px;font-size:10px;font-weight:900}.sm-side.ce{background:#dcfce7;color:#15803d;border:1px solid #86efac}.sm-side.pe{background:#fee2e2;color:#dc2626;border:1px solid #fca5a5}.sm-table-footer{height:42px;border-top:1px solid var(--sm-line);display:flex;align-items:center;justify-content:space-between;padding:0 14px;color:var(--sm-muted);font-size:12px}.sm-table-total{font-size:13px;font-weight:800}.sm-positive{color:var(--sm-green)!important}.sm-negative{color:var(--sm-red)!important}.sm-muted{color:var(--sm-muted)!important}.sm-tag{border-radius:4px;padding:4px 7px;font-size:10px;font-weight:800;background:#f1f5f9;color:#475569}.sm-tag.win,.sm-tag.buy,.sm-tag.ce,.sm-tag.closed{background:#ecfdf3;color:var(--sm-green)}.sm-tag.loss,.sm-tag.sell,.sm-tag.pe{background:#fef2f2;color:var(--sm-red)}.sm-tag.open{background:#eff6ff;color:var(--sm-blue)}.sm-cepe{color:#fff!important}.sm-cepe.ce{background:#0f3d91}.sm-cepe.pe{background:#b91c1c}.sm-table-empty{height:230px;text-align:center!important;color:var(--sm-muted)}.sm-instrument-body{display:grid;grid-template-columns:1fr 1fr;gap:0;padding:0 14px}.sm-instrument-note{margin:9px 14px 12px;padding-top:9px;border-top:1px solid var(--sm-line);font-size:11px;color:var(--sm-muted)}.sm-log-card{background:#fff}.sm-log-console{margin:0 8px 8px;background:var(--sm-console);border-radius:6px;color:#e5e7eb;min-height:0;flex:1;overflow:auto;padding:11px 12px;font:11px/1.6 ui-monospace,SFMono-Regular,Menlo,Monaco,Consolas,monospace}.sm-log-line{display:grid;grid-template-columns:76px 50px minmax(0,1fr);gap:8px;white-space:pre-wrap}.sm-log-line .INFO{color:#4ade80}.sm-log-line .WARN{color:#fbbf24}.sm-log-line .ERROR{color:#fb7185}.sm-log-line .DEBUG{color:#94a3b8}.sm-log-actions{display:flex;align-items:center;gap:7px}.sm-resume{height:29px;font-size:11px;padding:0 9px}.sm-history-btn,.sm-link-btn{height:31px;color:var(--sm-blue);border-color:#b8d2ff}.sm-footer-note{text-align:center;font-size:12px;color:var(--sm-muted);margin:14px 0 0}.sm-modal{position:fixed;inset:0;background:rgba(15,23,42,.5);z-index:1200;display:none;padding:24px}.sm-modal.open{display:grid;place-items:center}.sm-modal-panel{width:min(1180px,100%);max-height:88vh;background:#fff;border-radius:8px;box-shadow:0 24px 60px rgba(15,23,42,.24);display:flex;flex-direction:column;overflow:hidden}.sm-modal-head{display:flex;align-items:center;justify-content:space-between;padding:14px 16px;border-bottom:1px solid var(--sm-line)}.sm-modal-head h2{margin:0;font-size:17px}.sm-close{border:0;background:#f1f5f9;width:34px;height:34px;border-radius:50%;cursor:pointer;font-size:19px}.sm-history-tabs{display:flex;gap:6px;padding:12px 16px;border-bottom:1px solid var(--sm-line);overflow-x:auto}.sm-history-tabs button{height:34px;border:1px solid var(--sm-line);border-radius:6px;background:#fff;padding:0 13px;font-size:12px;font-weight:750;cursor:pointer}.sm-history-tabs button.active{background:#edf4ff;color:var(--sm-blue);border-color:#9fc0ff}.sm-history-body{padding:16px;overflow:auto;font-size:13px}.sm-history-state{border:1px dashed var(--sm-line);border-radius:7px;padding:30px;text-align:center;color:var(--sm-muted)}.sm-loading{opacity:.72}
    ...
    .sm-winner-card{min-width:0;padding:11px 16px;border:1px solid rgba(234,179,8,.45);border-radius:10px;background:linear-gradient(135deg,rgba(26,23,9,.96),rgba(11,18,36,.98));box-shadow:inset 0 1px 0 rgba(255,255,255,.08),0 0 20px rgba(234,179,8,.12);display:flex;align-items:center;justify-content:space-between;gap:14px}
    .sm-winner-period{display:flex;align-items:center;gap:8px;color:#fde047;font-size:11px;font-weight:900;letter-spacing:.03em;text-transform:uppercase}
    .sm-winner-period .star{color:#fde047;font-size:15px}
    .sm-winner-main{display:flex;align-items:center;justify-content:space-between;gap:18px;flex:1;min-width:0}
    .sm-winner-name{min-width:0;color:#f8fafc;font-size:14px;font-weight:800;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
    .sm-winner-type{display:inline-flex;margin-left:8px;padding:2px 6px;border:1px solid rgba(234,179,8,.4);border-radius:4px;color:#fef08a;font-size:9.5px;font-weight:850;vertical-align:1px}
    .sm-winner-values{flex:0 0 auto;display:flex;align-items:baseline;gap:12px;text-align:right}
    .sm-winner-return{color:#34d399;font-size:16px;font-weight:900;font-variant-numeric:tabular-nums}
    .sm-winner-pnl{color:#cbd5e1;font-size:11px;font-weight:750}
    .sm-winner-empty{color:#64748b;font-size:12px}
    ...
    .sm-key-tile{position:relative;isolation:isolate;min-height:160px;padding:12px 13px 11px;border:1px solid rgba(128,148,180,.7);border-radius:10px;background:linear-gradient(145deg,rgba(14,22,38,.99),rgba(4,9,19,.99));color:#94a3b8;box-shadow:inset 0 1px 0 rgba(255,255,255,.07),inset 0 -12px 22px rgba(0,0,0,.24),0 8px 24px rgba(0,0,0,.3);cursor:pointer;text-align:left;display:flex;flex-direction:column;transition:border-color .16s ease,transform .16s ease,box-shadow .16s ease,filter .16s ease;min-width:0;overflow:hidden}
    ...
    .sm-key-head{position:relative;z-index:2;display:flex;align-items:center;justify-content:space-between;gap:8px}
    .sm-key-name{font-size:13px;line-height:1.2;font-weight:800;color:#f8fafc;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
    .sm-key-instrument{flex:0 0 auto;padding:2px 6px;border-radius:4px;background:rgba(5,12,23,.85);border:1px solid rgba(148,163,184,.3);color:#dbeafe;font-size:9.5px;font-weight:850}
    .sm-key-pnl{position:relative;z-index:2;margin-top:10px;font-size:22px;line-height:1;font-weight:800;color:#aeb8ca;white-space:nowrap;text-align:center}
    ...
    .sm-key-return{position:relative;z-index:2;margin-top:4px;font-size:12px;font-weight:700;color:#aeb8ca;text-align:center}
    .sm-key-captured{position:relative;z-index:2;margin-top:3px;color:#dbeafe;font-size:10.5px;font-weight:800;text-align:center}.sm-key-captured.positive{color:#86efac}.sm-key-captured.negative{color:#fda4af}
    .sm-key-contract-pnl{position:relative;z-index:2;margin-top:8px;padding:5px 7px;border-radius:6px;background:rgba(8,16,32,.85);border:1px solid rgba(148,163,184,.18);display:grid;grid-template-columns:1fr 1fr;gap:6px}
    .sm-pnl-col{min-width:0;display:flex;flex-direction:column;gap:2px}
    .sm-pnl-col:last-child{padding-left:6px;border-left:1px solid rgba(148,163,184,.18)}
    .sm-pnl-col span{display:block;color:#93c5fd;font-size:8.5px;font-weight:850;letter-spacing:.03em;text-transform:uppercase;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
    .sm-pnl-col b{display:block;font-size:11px;font-weight:850;font-variant-numeric:tabular-nums;color:#f8fafc;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
    .sm-pnl-col b small{font-size:9px;font-weight:750;opacity:.85;margin-left:2px}
    .sm-pnl-col.positive b{color:#34d399}
    .sm-pnl-col.negative b{color:#fb7185}
    .sm-pnl-col.flat b{color:#cbd5e1}
    ...
    .sm-key-bottom{position:relative;z-index:2;display:flex;align-items:center;justify-content:space-between;gap:7px;margin-top:auto;padding-top:7px;color:#e2e8f0;font-size:10px;font-weight:700;text-transform:uppercase}
...
<body>
    ...
    <div class="sm-shell" id="shadowMonitor">
      <header class="sm-head">
        ...
        <div class="sm-head-actions">
          ...
          <a href="/tradeops" target="_blank" rel="noopener noreferrer" class="sm-tradeops-btn" title="Open TradeOps Live Money Trading Platform"><svg class="sm-icon" viewBox="0 0 24 24"><polygon points="13 2 3 14 12 14 11 22 21 10 12 10 13 2"/></svg><span>⚡ TradeOps (Real Money)</span></a>
      ...
      <section class="sm-consolidated sm-consolidated-view" id="consolidatedView" hidden>
        <div class="sm-consolidated-toolbar"><div class="sm-consolidated-toolbar-title"><b id="consolidatedInstrumentTitle">Futures Consolidated</b><span id="consolidatedInstrumentMeta">All Futures shadow strategies</span></div><div style="display:flex;align-items:center;gap:10px"><div class="sm-consolidated-instrument" role="tablist" aria-label="Consolidated instrument"><button type="button" class="active" data-consolidated-instrument="FUTURES">Futures</button><button type="button" data-consolidated-instrument="OPTIONS">Options</button></div><a href="/tradeops" target="_blank" rel="noopener noreferrer" class="sm-consolidated-tradeops-btn" title="Open TradeOps Live Money Trading Platform"><svg class="sm-icon" viewBox="0 0 24 24"><polygon points="13 2 3 14 12 14 11 22 21 10 12 10 13 2"/></svg><span>TradeOps Live Trading &rarr;</span></a></div></div>
  ...
  <script>
    (function(){
      ...
      function rankCard(row,label,icon){if(!row)return'<article class="sm-winner-card"><div class="sm-winner-period"><span class="star">'+(icon||"★")+'</span>'+esc(label)+'</div><div class="sm-winner-empty">No completed trades today</div></article>';var positive=Number(row.returnPct)>=0;return'<article class="sm-winner-card"><div class="sm-winner-period"><span class="star">'+(icon||"★")+'</span>'+esc(label)+'</div><div class="sm-winner-main"><div class="sm-winner-name">'+esc(row.strategyName)+'<span class="sm-winner-type">'+esc(row.instrument==="FUTURES"?"FUT":"OPT")+'</span></div><div class="sm-winner-values"><span class="sm-winner-return '+(positive?"sm-positive":"sm-negative")+'">'+pct(row.returnPct)+'</span><span class="sm-winner-pnl">'+money(row.pnl)+' · '+money(row.capitalUsed).replace(/^\\+/,"")+' capital</span></div></div></article>'}
      function consolidatedTileMarkup(t){var key=consolidatedKey(t);return'<button class="sm-key-tile" type="button" data-tile-key="'+esc(key)+'" data-strategy="'+esc(t.strategyId)+'" data-instrument="'+esc(t.instrumentType)+'"><span class="sm-key-select-mark" aria-hidden="true" title="Top performer today">&#9733;</span><div class="sm-key-head"><span class="sm-key-name"></span><span class="sm-key-instrument"></span></div><div class="sm-key-pnl"></div><div class="sm-key-return"></div><div class="sm-key-captured"></div><div class="sm-key-contract-pnl"><div class="sm-pnl-col sm-cycle-curr"><span>Current</span><b>--</b></div><div class="sm-pnl-col sm-cycle-prev"><span>Last Month</span><b>--</b></div></div><svg class="sm-key-spark" viewBox="0 0 320 58" preserveAspectRatio="none" aria-hidden="true"><polyline points="0,43 18,38 34,42 50,34 68,36 86,30 103,33 120,28 138,31 156,24 175,27 193,20 211,23 230,17 248,19 266,12 284,15 302,8 320,4"/></svg><div class="sm-key-bottom"><span class="sm-key-trades"></span><span class="sm-key-state"></span></div></button>'}
      function consolidatedGroupMarkup(type,tiles){
    var CORE_IDS=["tt1000","tt1000-quality-breakout","tt1000-unlimited","tt1030","tt1030-quality-reversal","tt1030-unlimited"];
    var coreTiles=tiles.filter(function(t){return CORE_IDS.indexOf(t.strategyId)>=0});
    var otherTiles=tiles.filter(function(t){return CORE_IDS.indexOf(t.strategyId)<0});
    var label=type==="FUTURES"?"Futures":"Options";
    var html="";
    if(coreTiles.length>0){
      html+='<section class="sm-instrument-group core-group '+type.toLowerCase()+'" data-instrument-group="'+type+'-core"><header class="sm-instrument-group-head"><h2 class="sm-instrument-group-title"><span style="color:#6366f1;margin-right:6px">&#9889;</span>Core Breakout Strategies ('+label+')</h2><span class="sm-instrument-group-summary" data-group-summary="'+type+'-core"></span></header><div class="sm-consolidated-grid">'+coreTiles.map(consolidatedTileMarkup).join("")+'</div></section>';
    }
    if(otherTiles.length>0){
      html+='<section class="sm-instrument-group other-group '+type.toLowerCase()+'" style="margin-top:24px" data-instrument-group="'+type+'-other"><header class="sm-instrument-group-head"><h2 class="sm-instrument-group-title"><span style="color:#a855f7;margin-right:6px">&#128202;</span>Other Shadow Strategies ('+label+')</h2><span class="sm-instrument-group-summary" data-group-summary="'+type+'-other"></span></header><div class="sm-consolidated-grid">'+otherTiles.map(consolidatedTileMarkup).join("")+'</div></section>';
    }
...
var coreTotal=coreTiles.reduce(function(sum,t){return sum+(Number(t.pnl)||0)},0);
var coreTrades=coreTiles.reduce(function(sum,t){return sum+(Number(t.trades)||0)},0);
var coreSum=grid.querySelector('[data-group-summary="'+selectedInstrument+'-core"]');
if(coreSum)coreSum.innerHTML=coreTiles.length+' strategies &middot; <b class="'+clsPnl(coreTotal)+'">'+money(coreTotal)+'</b> &middot; '+coreTrades+' trades';
var otherTotal=otherTiles.reduce(function(sum,t){return sum+(Number(t.pnl)||0)},0);
