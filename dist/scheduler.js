...
exports.autoPaperTradeFromPicks = autoPaperTradeFromPicks;
exports.monitorAutoPaperPositions = monitorAutoPaperPositions;
exports.squareOffIntradayPositions = squareOffIntradayPositions;
exports.checkPriceAlerts = checkPriceAlerts;
exports.sendMorningReminder = sendMorningReminder;
exports.sendEODSummary = sendEODSummary;
exports.trackPickResults = trackPickResults;
exports.sendWeeklyAdminSummary = sendWeeklyAdminSummary;
...
async function refreshPrices() {
    ...
    const rows = await (0, nse_1.fetchLatestBhavcopy)();
    if (rows.length === 0) {
    ...
    const existingSet = new Set(await (0, db_1.getAllSymbols)());
    const newSymbols = rows.filter(r => !existingSet.has(r.symbol));
    for (const r of newSymbols) {
        await (0, db_1.dbRun)("INSERT OR IGNORE INTO stocks (symbol) VALUES (?)", [r.symbol]);
    }
    ...
    for (const symbol of targets) {
        try {
            ...
            if (f.error && f.error.includes("Not found")) {
                await (0, db_1.upsertStock)({ symbol, fetch_error: f.error, fetched_at: new Date().toISOString() });
                errors++;
            }
            else if (!f.error) {
                await (0, db_1.upsertStock)({
                    symbol,
                ...
                if (done % 20 === 0)
                    console.log(`[Scheduler] ${done}/${targets.length} done, ${errors} errors`);
            }
        }
        catch (e) {
...
async function checkAlerts() {
    ...
    for (const alert of alerts) {
        ...
        try {
            ...
            if (stocks.length > 0) {
                await (0, mailer_1.sendAlertEmail)(alert.user_email, alert.user_name, alert.name, stocks);
                await (0, db_1.updateAlertLastSent)(alert.id);
                sent++;
            }
        }
        catch (e) {
...
async function generateDailyPicks() {
    ...
    await (0, db_1.dbRun)(`UPDATE picks SET status='expired' WHERE status='active' AND created_by IS NULL AND date(published_at) < ?`, [today]);
    // Check if auto-picks already generated today
    const existing = await (0, db_1.dbAll)(`SELECT id FROM picks WHERE status='active' AND created_by IS NULL AND date(published_at) = ?`, [today]);
    ...
    const stocks = await (0, db_1.dbAll)(`
  ...
  `);
    ...
    function selectBest(pool, maxPicks, maxPerSector, usedSymbols) {
        ...
        const result = [];
        for (const s of pool) {
            if (result.length >= maxPicks)
                break;
            if (s.score < medianScore)
                break;
    ...
    function riskLevel(drPct, de) {
        if (drPct > 3.5 || (de != null && de > 1.5))
            return "high";
        if (drPct > 1.8 || (de != null && de > 0.7))
            return "medium";
        return "low";
    }
    ...
    const scalperPool = stocks
        ...
        .sort((a, b) => b.score - a.score);
    let scalperCount = 0;
    for (const s of selectBest(scalperPool, 10, 3, new Set())) {
        const price = s.price;
        const dir = s.isLong ? "LONG" : "SHORT";
        ...
        parts.push(`Volume ${(s.volume / 1e6).toFixed(1)}M — high liquidity scalp`);
...
async function autoPaperTradeFromPicks() {
    ...
    const picks = await (0, db_1.dbAll)(`SELECT * FROM picks WHERE status='active' AND pick_type != 'longterm' AND date(published_at) >= date('now','localtime','-2 days')`);
    if (picks.length === 0) {
        console.log("[AutoPaper] No picks available today");
        return;
    }
        ...
        for (const pick of picks) {
            ...
            if (result.ok) {
                ...
                if (pick.result !== 'entry_triggered') {
                    await (0, db_1.updatePickEntry)(pick.id, price).catch(() => { });
                }
            }
            else {
        ...
        for (const pos of positions) {
            ...
            const hit = (pos.target_price && livePrice >= pos.target_price) ? "TARGET" :
                (pos.sl_price && livePrice <= pos.sl_price) ? "STOPLOSS" :
                    null;
            if (hit) {
                const result = await (0, db_1.paperSell)(user.id, pos.symbol, pos.qty, livePrice);
...
async function squareOffIntradayPositions() {
    ...
    for (const user of users) {
        const positions = await (0, db_1.getPaperPositions)(user.id);
        const intradayPos = positions.filter((p) => p.trade_type === 'INTRADAY');
        if (intradayPos.length === 0)
            continue;
        let closed = 0;
...
async function checkPriceAlerts() {
    const activeAlerts = await (0, db_1.getAllActivePriceAlerts)();
    let triggered = 0;
    for (const a of activeAlerts) {
        ...
        if (!hit)
            continue;
        try {
            await (0, db_1.triggerPriceAlert)(a.id);
            await (0, mailer_1.sendPriceAlertEmail)(a.user_email, a.user_name, a.symbol, a.direction, a.target_price, a.current_price);
            triggered++;
        }
        catch (e) {
    ...
    if (triggered > 0)
        console.log(`[PriceAlerts] Triggered ${triggered}/${activeAlerts.length} alerts`);
}
// ── Telegram helper ───────────────────────────────────────────────────────────
...
async function sendMorningReminder() {
    ...
    // Today's picks
    const picks = await (0, db_1.dbAll)(`SELECT stock_symbol, pick_type, direction, reason FROM picks WHERE status='active' AND date(published_at)=? AND pick_type != 'longterm' ORDER BY pick_type`, [today]);
    ...
    const msg = `⏰ *Good Morning — Market opens in 45 mins*\n` +
        `━━━━━━━━━━━━━━━━━━━━━\n` +
        `${yLine}\n\n` +
...
async function sendEODSummary() {
    ...
    const tmrPicks = await (0, db_1.dbAll)(`SELECT stock_symbol, pick_type, direction, reason FROM picks WHERE status='active' AND date(published_at)=? AND pick_type != 'longterm' ORDER BY pick_type LIMIT 8`, [today]);
...
function startScheduler() {
    ...
    node_cron_1.default.schedule("15 13 * * 1-5", async () => {
    ...
    node_cron_1.default.schedule("45 3 * * 1-5", async () => {
    ...
    node_cron_1.default.schedule("*/5 3-10 * * 1-5", async () => {
        await monitorAutoPaperPositions();
    }, { timezone: "UTC" });
    // Intraday forced square-off: 3:15 PM IST = 09:45 UTC, weekdays
    node_cron_1.default.schedule("45 9 * * 1-5", async () => {
    ...
    node_cron_1.default.schedule("*/30 3-10 * * 1-5", async () => {
    ...
    node_cron_1.default.schedule("0 3 * * 1-5", async () => {
