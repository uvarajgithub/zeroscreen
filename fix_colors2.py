...
# Get the em-dash equivalent char from the file
emdash = None
for ln in lines[start:end+1]:
    if "'+'" in ln and "Γé" in ln:
        idx = ln.find("Γé")
        emdash_candidate = ln[idx:idx+3]
        print(f'Found rupee-like: {repr(emdash_candidate)}')
        break
...
NEW_FUNC = [
    ...
    "            +'<b style=\"color:'+_tc+';font-size:.72rem\">'+(_ti>=0?'+':'-')+'&#8377;'+Math.abs(_ti)+'</b>'\n",
    "            +_re\n",
    "            +'</div>';\n",
    "        });\n",
    "        _ch+='</div>';\n",
...
# Now fix ss-trade-breakdown section too — find and update its forEach
for i, ln in enumerate(lines):
    if "_bdHtml+='<div>Trade '" in ln and '_td+' in ln:
        print(f'Found breakdown line at {i+1}: {repr(ln[:80])}')
        lines[i] = (
            "              _bdHtml+='<div style=\"display:flex;align-items:center;gap:6px;padding:2px 0;font-size:.65rem\">'\n"
            "                +'<span style=\"color:#64748b\">T'+(i+1)+'</span>'\n"
            "                +(_td?'<span style=\"color:'+(_ti>=0?'#4ade80':'#fb923c')+';font-weight:700\">'+(_ti>=0?'+':'-')+'&#8377;'+Math.abs(_ti)+'</span>':'')\n"
            "                +_td\n"
            "                +'<span style=\"color:#64748b\">'+_tm+'</span>'\n"
            "                +(_tp?'<span style=\"color:#475569\">'+_tp+'</span>':'')\n"
            "                +'</div>';\n"
        )
        print('Updated breakdown line')
        break
