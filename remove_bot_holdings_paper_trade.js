// remove_bot_holdings_from_paper_trade.js — remove Bot Holdings from /paper-trade page
const fs = require('fs');
const p = '/root/zeroscreen/dist/server.js';
let s = fs.readFileSync(p, 'utf8');

const OLD = `
      <!-- Bot Holdings (OPEN Penny positions) -->
      ...
      <div style="font-size:0.72rem;font-weight:700;text-transform:uppercase;letter-spacing:.1em;color:#f59e0b;border-bottom:1px solid var(--border);padding-bottom:8px;margin-bottom:12px;margin-top:24px">
        ╔ᆱ╞Æ├┤├ Bot Holdings &mdash; Long Term &nbsp;<span style="font-weight:400;color:var(--text-muted)">(paper · auto-exit on SL/Target)</span>
...
if (!s.includes(OLD)) {
  // Try reading exact bytes from file
  const idx = s.indexOf('<!-- Bot Holdings (OPEN Penny positions) -->');
  if (idx < 0) { console.error('ERROR: Bot Holdings block not found'); process.exit(1); }
  // Find end marker
  const endMarker = "` : ''}";
  let searchFrom = idx;
  let endIdx = -1;
  // Find the closing ` : ''} after the block
  for (let i = idx; i < s.length - 10; i++) {
    if (s.substring(i, i + 7) === "` : ''}") {
      endIdx = i + 7;
      break;
    }
  }
  if (endIdx < 0) { console.error('ERROR: could not find block end'); process.exit(1); }
  s = s.substring(0, idx - 7) + s.substring(endIdx); // -7 to include the leading newline+spaces
  console.log('Removed Bot Holdings block via index method');
} else {
  s = s.replace(OLD, '');
  console.log('Removed Bot Holdings block via string replace');
}

fs.writeFileSync(p, s);
console.log('Done: Bot Holdings removed from /paper-trade page');
