 ...
 * ║  Created : 2026-05-17                                       ║
 * ╠══════════════════════════════════════════════════════════════╣
 * ║  Entry : AMINA early entry (C2 close)                       ║
 * ║    Rule A: C1+C2 same color                                 ║
 * ║      If C2.close breaks C1's level → enter at C2.close      ║
 * ║      Else wait for C3+ to break max(C1.h, C2.h)             ║
 * ║    Rule B: C2 body > C1 body (opposite colors)              ║
 * ║      If C2.close breaks C1's body level → enter at C2.close ║
 * ║      Else wait for C3+ to break combined body level          ║
 ...
 * ║  Net ₹  : ₹18,80,296  ✅ NEW BEST                           ║
 * ║  Win%   : 58.3%                                             ║
 * ║  MaxDD  : ₹15,297                                           ║
 * ║  Avg/day: ₹1,419                                            ║
 * ║  ALL 6 YEARS GREEN (2021–2026)                              ║
 * ╠══════════════════════════════════════════════════════════════╣
 * ║  vs AMINA SL60 baseline:  +₹4,56,273 (+32%)                ║
 * ║  vs AMINA-S7R50 (C3):     +₹2,39,631 (+14.6%)              ║
 * ╚══════════════════════════════════════════════════════════════╝
 */
...
// ── AMINA-E2: enter at C2.close if it already breaks C1's level ──────────────
function rollingEntryScan(cs) {
  for (let i = 0; i < cs.length - 1; i++) {
    ...
    // early: C2 itself breaks C1's level
...
function simDay(candles) {
  ...
  for (let idx = 0; idx < cs.length; idx++) {
    ...
    // ── SCANNING: wait for AMINA-E2 entry signal ──────────────────────────
    if (phase === 'SCANNING') {
      if (isEOD(c)) break;
...
(async () => {
  ...
  console.log('AMINA-E2');
  console.log('Entry  : AMINA early (C2 close if breaks C1 level, else C3+ confirm)');
  ...
  console.log(`AMINA-S7R50 (C3)    = ₹16,40,665`);
  console.log(`AMINA-E2    (C2)    = ₹${r.netRs.toLocaleString('en-IN')}  ${r.netRs > 1640665 ? '✅ NEW BEST' : r.netRs > 1424023 ? '✅ beats baseline' : '❌'}`);
  console.log(LINE);
})().catch(console.error);
