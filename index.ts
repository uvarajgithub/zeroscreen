import fs from "fs";
import { getPreviousCandle, getCurrentCandle, getTwoLastCandles, getStructureSeed, getSwingLevels, getLatest5MinCandle, getCurrentPrice, getBestOptionSymbol, getAvgCandleSize, getVWAP, getAvgVolume, getRecentCandles, getDayOpenPrice, getOptionDayOHLC, getOptionLTP, getITMMonthlyOptionSymbol } from "./market";
import { KiteConnect } from "kiteconnect";
import { getCandleBody, checkBreakout, isSideways, isStrongMomentum, isNearBreakout, isVwapAligned, isWeakMomentum, isStrongTrend, isBreakoutAccelerating, is15MinAligned, isBigMoveAlready, getTrailingSL, isWithinTime, isMomentumAligned, isHighWickCandle, isNearVwapChop, Candle, HybridReverseState, HybridSignal, createHybridState, processHybridCandle, trailLock50 } from "./strategy";
...
let shadowState:      HybridReverseState = createHybridState();
let shadowPrevCandle: Candle | null      = null;
let shadowPnL        = 0;
let shadowTrades     = 0;
    ...
    if (candleKey !== lastCandleKey) {
      ...
      if (ACTIVE_STRATEGY === "HYBRID_REVERSE") {
        ...
        const buildTriggerBlock = (
          ...
          const ceArrow = (trigCE - price) <= 20 ? "🔥" : "⬆️";
          const peArrow = (price - trigPE) <= 20 ? "🔥" : "⬇️";
          let lines = `━━━━━━━━━━━━━━━━━━\n📐 *Next candle needs to close at:*\n`;
          if (showBoth || onlyDir === "CE") lines += `${ceArrow} CE entry if close ≥ *${trigCE}*\n`;
          if (showBoth || onlyDir === "PE") lines += `${peArrow} PE entry if close ≤ *${trigPE}*\n`;
          lines += `   Live now: *${price}*`;
          return lines;
        };
        ...
        if (inTrade && tradeDirection) {
          ...
          strategyCtx =
            ...
            `Unrealised: *${pnlSign}${unrealised.toFixed(0)} pts*\n` +
            `Day P&L: ${dailyPnL >= 0 ? "+" : ""}${dailyPnL} pts | Trades: ${tradeCount}/5`;
          ...
          if (refLevel > 0) {
            ...
            strategyCtx =
              ...
              `(SL hit — same direction re-entry only)\n` +
              `━━━━━━━━━━━━━━━━━━\n` +
              `📐 *Next candle needs to close at:*\n` +
              `${arrow} ${dir} entry if close ${symbol} *${refLevel}*\n` +
              `   Live now: *${price}*  ${distStr}\n` +
              `Day P&L: ${dailyPnL >= 0 ? "+" : ""}${dailyPnL} pts | Trades: ${tradeCount}/5`;
          } else {
            strategyCtx =
              `━━━━━━━━━━━━━━━━━━\n` +
          ...
          if (prevCandle) {
            ...
            if (finalPrev.close > ceLvl) {
              ...
              sigStatus = `⬜ No signal this candle\n` +
                buildTriggerBlock(nextBH + HR_BUF_MON, nextBL - HR_BUF_MON, true);
            }
          } else {
            const nextBH = Math.max(finalPrev.open, finalPrev.close);
            const nextBL = Math.min(finalPrev.open, finalPrev.close);
            sigStatus = `⬜ First candle (no prior ref)\n` +
              buildTriggerBlock(nextBH + HR_BUF_MON, nextBL - HR_BUF_MON, true);
          }
          strategyCtx =
            `━━━━━━━━━━━━━━━━━━\n` +
      ...
      await sendTelegram(
        ...
        `\n━━━━━━━━━━━━━━━━━━\n` +
        `[Token Refresh](http://139.59.18.52/submit)  |  [Zeroscreen](http://139.59.18.52/dashboard)`
      );
...
function printStatus() {
  ...
  const tradeStatus = inTrade
    ...
    : "FLAT";
  const shadowSign = shadowPnL >= 0 ? "+" : "";
  console.log(`[${ist}] [${mode}] | TRAIL: ${pnlSign}${livePnL.toFixed(0)}pts T:${tradeCount}/${MAX_TRADES} ${tradeStatus} | LOCK50: ${shadowSign}${shadowPnL.toFixed(0)}pts T:${shadowTrades}/${MAX_TRADES}`);
}

// ─── Main Loop ───────────────────────────────────────────
let runBotActive = false; // prevents concurrent runBot calls stacking up when API hangs
let tokenAlertSent = false; // one-time token-expired Telegram alert guard
...
async function runHybridReverseBot() {
  ...
  if (h === 9 && m === 15) {
    ...
    entryPrice            = 0;
    entryTime             = 0;
    // Reset shadow LOCK50 state
    shadowState       = createHybridState();
    shadowPrevCandle  = null;
    shadowPnL         = 0;
    shadowTrades      = 0;
  ...
  const sig: HybridSignal = processHybridCandle(hybridState, hybridPrevCandle, currentCandle, isEOD);
  hybridPrevCandle = currentCandle;  // advance reference

  // ── Shadow LOCK50: same candle, same entry logic, different trail ──────────
  if (shadowPrevCandle) {
    const shadowSig = processHybridCandle(shadowState, shadowPrevCandle, currentCandle, isEOD, trailLock50);
    if (shadowSig.action === "ENTER" || shadowSig.action === "REVERSE_ENTER") shadowTrades++;
    if (shadowSig.action === "EXIT_EARLY" || shadowSig.action === "EXIT_SL" || shadowSig.action === "EXIT_EOD") {
      shadowPnL += shadowSig.pts;
    }
  }
...
(async () => {
  ...
  try {
    ...
    setInterval(() => {
      ...
      runBot()
        .then(() => { tokenAlertSent = false; }) // reset on successful cycle so tomorrow's expiry re-alerts
        .catch(err => {
    ...
    if (restored && activeTrade && tradeDirection && entryPrice > 0) {
      ...
      sendTelegram(
        ...
        `🔑 [Refresh Token](http://139.59.18.52/submit)`
      ...
      sendTelegram(
        ...
        `Day P&L: ${dailyPnL >= 0 ? "+" : ""}${dailyPnL} pts | Trades: ${tradeCount}/5\n` +
        `Mode: ${config.mode.toUpperCase()} | Qty: ${config.quantity}\n` +
        `[Token Refresh](http://139.59.18.52/submit)`
      ).catch(e => console.error("[Telegram restart notify failed]", e?.message ?? e));
    } else {
      ...
      sendTelegram(
        ...
        `⏰ ${new Date().toLocaleTimeString("en-IN", { timeZone: "Asia/Kolkata" })}\n` +
        `🔑 [Refresh Token if needed](http://139.59.18.52/submit)`
