...
// reMode: 'none' | 'opposite' | 'same' | 'nextBreakout'
function simDay(rawcs, reMode){
  ...
  for(let idx=0;idx<cs.length;idx++){
    ...
    if(t1.type==='SL'&&reMode!=='none'){
      let reDir=null;
      if(reMode==='opposite') reDir=res.sig==='CE'?'PE':'CE';
      if(reMode==='same')     reDir=res.sig;
      if(reMode==='nextBreakout'){
        // scan from exit candle onwards for next rolling entry signal
        const remaining=cs.slice(t1.exitIdx);
        const nextRes=rollingEntryScan(remaining);
        if(nextRes) reDir=nextRes.sig;
      }
      if(reDir){
        const re=simLeg(cs,t1.exitIdx,reDir);
        rePts=re.pts;
      }
    }
    return t1.pts+rePts;
...
const variants=[
  ...
  {k:'D',label:'RE next breakout signal     ',mode:'nextBreakout'},
];
...
console.log(L);
console.log('  5-YEAR RE ENTRY COMPARISON  (all: buf=25, trail=100)');
