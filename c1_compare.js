// C1 early-exit comparison: 4 variants over 5 years
// Option 1: No C1 (remove entirely)
// Option 2: C1=20pts (raise threshold)
// Option 3: C1 on first entry only (disabled on re-entries)
// Current: C1=3pts (live bot as-is)
...
// c1Mode: 'none' | 'all' | 'firstOnly'
// c1Pts: threshold in pts (e.g. 3 or 20)
function processHybrid(state, prev, curr, isEOD, c1Mode, c1Pts) {
  ...
  if(state.inTrade){
    ...
    if(state.isC1){
      state.isC1=false;
      const applyC1 = (c1Mode==='all') || (c1Mode==='firstOnly' && !state.reUsed);
      if(applyC1){
        ...
        if(p<-c1Pts){
          ...
          return{action:'EXIT_EARLY',pts:p};
        }
      }
    }
    // SL / reverse check
...
function runStrategy(allDates, byDay, c1Mode, c1Pts) {
  ...
  for(const date of allDates){
    ...
    for(let i=1;i<candles.length;i++){
      ...
      const sig=processHybrid(state,prev,curr,isEOD,c1Mode,c1Pts);
      if(sig.action==='ENTER'){trades++;}
        ...
        dayPnl+=sig.pts;totalTrades++;if(sig.pts>0)winTrades++;
      }
    }
    totalPts+=dayPnl;
...
async function main() {
  ...
  const strategies = [
    { name:'No C1  (Option 1 — recommended)', c1Mode:'none',      c1Pts:0  },
    { name:'C1=20pts  (Option 2)',             c1Mode:'all',       c1Pts:20 },
    { name:'C1 first-entry only  (Option 3)',  c1Mode:'firstOnly', c1Pts:3  },
    { name:'Current  (C1=3pts LIVE)',          c1Mode:'all',       c1Pts:3  },
  ];

  const results = strategies.map(s => ({ ...s, ...runStrategy(allDates, byDay, s.c1Mode, s.c1Pts) }));
  results.sort((a,b)=>b.totalPts-a.totalPts);

  const SEP='─'.repeat(120);
  console.log(SEP);
  console.log(`  ${'#'.padEnd(2)}  ${'Strategy'.padEnd(32)}  ${'TotalPts'.padStart(9)}  ${'Total ₹'.padStart(12)}  ${'PremPts'.padStart(8)}  ${'WinDay%'.padStart(8)}  ${'MaxDD pts'.padStart(10)}  ${'MaxDD ₹'.padStart(10)}  ${'Avg/day'.padStart(8)}`);
  console.log(SEP);
  ...
  results.forEach((r,i) => {
    ...
    console.log(
      `  ${String(i+1).padEnd(2)}  ${r.name.padEnd(32)}  ${(s+r.totalPts.toLocaleString('en-IN')).padStart(9)}  ${(s+'₹'+Math.abs(r.totalRs).toLocaleString('en-IN')).padStart(12)}  ${(s+r.premPts.toLocaleString('en-IN')).padStart(8)}  ${(r.winDayPct+'%').padStart(8)}  ${String(dd).padStart(10)}  ${String(ddR).padStart(10)}  ${avg.padStart(8)}`
    );
  });
  console.log(SEP);

  console.log(`\n  Trade-level stats:`);
  console.log(`  ${'Strategy'.padEnd(32)}  ${'Total Trades'.padStart(13)}  ${'Win Trades'.padStart(11)}  ${'Trade Win%'.padStart(11)}  ${'Win Days'.padStart(9)}  ${'Loss Days'.padStart(10)}`);
  console.log(SEP);
  results.forEach(r => {
    console.log(`  ${r.name.padEnd(32)}  ${String(r.totalTrades).padStart(13)}  ${String(r.winTrades).padStart(11)}  ${(r.tradeWinPct+'%').padStart(11)}  ${String(r.winDays).padStart(9)}  ${String(r.lossDays).padStart(10)}`);
  });
  console.log(SEP);
  console.log(`\n  QM=₹${QM}/pt  |  All use trailLock50 + buf=25 + unlimited trades  |  ${allDates.length} trading days`);
}
