'use strict';
// ════════════════════════════════════════════════════════════════════════════
// backtest_honest_close.js — BHAV ABLATION: FULLY HONEST EXIT PRICES
// ════════════════════════════════════════════════════════════════════════════
// SAME as backtest_ablation.js but ALL exits use actual candle close price.
// No trailStop level credit — shows TRUE P&L without stop-order inflation.
//
// Exit logic:
//   TRAIL: candle-close check, exit at ACTUAL CLOSE (honest)
//   SL:    candle-close check, exit at ACTUAL CLOSE (honest)
//   This is the conservative floor — what you get if no SLM orders are placed.
//
...
function calcPL(candles, entryIdx, side, tGap) {
  ...
  for (let i = entryIdx + 1; i < candles.length; i++) {
    ...
    if (closePts <= trailStop) {
      // ALL exits: actual close price (honest — no stop-level inflation)
...
console.log('  BHAV STRATEGY — ENTRY FILTER ABLATION STUDY');
  console.log('  Exit: ALL exits at ACTUAL CANDLE CLOSE (fully honest — no stop-order inflation)');
