'use strict';
// Counts how many days entry was at C0 (9:30), C1 (9:45), C2+ (later), or no entry
...
function getFirstEntryIdx(today, prev) {
  const liveCandles = today.slice(1);
  if (liveCandles.length < 2) return null;
  const DAILY_LOSS_CAP_LOCAL = 150;
  let firstDone = false;
  for (let li = 0; li < liveCandles.length - 1; li++) {
    if (firstDone) return null; // only care about first entry
    const sliceNow = liveCandles.slice(0, li + 1);
    const sig = findDrishtiEntry(sliceNow, prev);
    if (sig && sig.idx === li) return { li, reason: sig.reason, ctx: sig.ctx };
  }
  return null;
}
...
async function main(){
  console.log('Fetching 5yr candle data...');
  const startDate=new Date('2021-01-01'),endDate=new Date('2026-05-25');
  const all=[];
  let cur=new Date(startDate);
  while(cur<endDate){
    const chunkEnd=new Date(cur);chunkEnd.setDate(chunkEnd.getDate()+59);
    if(chunkEnd>endDate)chunkEnd.setTime(endDate.getTime());
    const from=cur.toISOString().slice(0,10),to=chunkEnd.toISOString().slice(0,10);
  ...
  // Count entry candle distribution
  const counts = { C0:0, C1:0, C2:0, C3:0, C4:0, C5to8:0, C9plus:0, noEntry:0 };
  const reasonCount = {};
  const ctxCount = { ABOVE_PDH:0, BELOW_PDL:0, INSIDE:0 };
  const c0Days = [], c1Days = [];
  ...
  for(let di=1;di<allDates.length;di++){
    ...
    const res=getFirstEntryIdx(today,prev);
    if(!res){counts.noEntry++;continue;}
    const {li,reason,ctx}=res;
    // li=0 → C0 (9:30), li=1 → C1 (9:45), etc.
    if(li===0)     {counts.C0++;c0Days.push({date,reason});}
    else if(li===1){counts.C1++;c1Days.push({date,reason});}
    else if(li===2) counts.C2++;
    else if(li===3) counts.C3++;
    else if(li===4) counts.C4++;
    else if(li<=8)  counts.C5to8++;
    else            counts.C9plus++;
    reasonCount[reason]=(reasonCount[reason]||0)+1;
    if(ctx)ctxCount[ctx]=(ctxCount[ctx]||0)+1;
  }

  const total=Object.values(counts).reduce((a,b)=>a+b,0);
  console.log('=== ENTRY CANDLE DISTRIBUTION (5yr, 1331 days) ===\n');
  const timeMap={C0:'9:30',C1:'9:45',C2:'10:00',C3:'10:15',C4:'10:30',C5to8:'10:45–11:45',C9plus:'12:00+',noEntry:'No entry'};
  for(const [k,v] of Object.entries(counts)){
    const pct=(v/total*100).toFixed(1);
    const bar='█'.repeat(Math.round(v/total*40));
    console.log(`${(k+' ('+timeMap[k]+')').padEnd(28)} ${String(v).padStart(4)} days  ${pct.padStart(5)}%  ${bar}`);
  }

  console.log('\n=== ENTRY REASON BREAKDOWN ===');
  Object.entries(reasonCount).sort((a,b)=>b[1]-a[1]).forEach(([r,v])=>{
    console.log(`  ${String(v).padStart(4)}  ${r}`);
  });

  console.log('\n=== CONTEXT BREAKDOWN ===');
  Object.entries(ctxCount).forEach(([c,v])=>console.log(`  ${c}: ${v} days`));

  console.log('\n=== SAMPLE C0 ENTRY DAYS (first 10) ===');
  c0Days.slice(0,10).forEach(d=>console.log(`  ${d.date}  ${d.reason}`));
}
